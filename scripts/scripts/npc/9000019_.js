...
function action(mode, type, selection) {
  if (mode != 1) {
    if (status == 1) cm.sendOk("Why of course, you're tot chicken to face me in Rock, Paper, Scissors!");
    cm.dispose();
    return;
  } else status++;
  if (status == 0) {
    ...
    if (losematch == true) cm.gainMeso(-5000);
    if (winmatch == true) cm.gainMeso(5000);
    cm.dispose();
  }
}
