  ...
  if (status == 0 && mode == 1) {
    ...
    if (cm.getParty() == null) {
      ...
      if (cm.isLeader()) {
        ...
        if (pt.getMembers().size() < 2) {
          if (
            (cm.getParty() != null && 1 < cm.getParty().getMembers().size() && cm.getParty().getMembers().size() < (selection == 4 || selection == 5 || selection == 8 ? 4 : 3)) ||
            cm.getPlayer().isGM()
          ) {
    ...
    if (selection == 100) {
      ...
      if (cm.getEventManager('cpq2').getInstance('cpq' + mapid) == null) {
        ...
        cm.getParty() != null &&
        cm
          .getEventManager('cpq2')
          .getInstance('cpq' + mapid)
          .getPlayerCount() == cm.getParty().getMembers().size()
      ) {
        if (checkLevelsAndMap(51, 120) == 1) {
          cm.sendOk('队伍里有人等级不符合。');
          cm.dispose();
