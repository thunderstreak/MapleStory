  ...
  if (status == 0) {
    ...
    if (selection == 1) {
      ...
      if (victim.getLevel() > 69) {
        ...
        Packages.handling.world.World.Broadcast.broadcastMessage(
          Packages.tools.MaplePacketCreator.serverNotice(
            11,
            cm.getC().getChannel(),
            '师徒系统' + ' : [' + cm.getPlayer().getName() + ']收了[' + victim.getName() + ']为徒弟，请徒弟进行确认！',
            true
          ).getBytes()
        );
        cm.dispose();
      ...
      if (cm.isLeader()) {
        ...
        Packages.handling.world.World.Broadcast.broadcastMessage(
          Packages.tools.MaplePacketCreator.serverNotice(
            11,
            cm.getC().getChannel(),
            '师徒系统' + ' : [' + cm.getPlayer().getName() + ']玩家成功带徒出师[' + victim.getName() + ']获得重磅礼包！',
            true
          ).getBytes()
        );
        cm.dispose();
      ...
      if (cm.getPlayer().getLevel() >= 200) {
        ...
        cm.sendOk('师徒关系确认成功！');
        Packages.handling.world.World.Broadcast.broadcastMessage(
          Packages.tools.MaplePacketCreator.serverNotice(11, cm.getC().getChannel(), '师徒系统' + ' : [' + cm.getPlayer().getName() + ']师徒关系确认成功。', true).getBytes()
        );
        cm.dispose();
      } else {
