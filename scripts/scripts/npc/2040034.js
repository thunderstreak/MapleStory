  ...
  if (mode == -1) {
    ...
    if (status == 0) {
      ...
      if (selection == 0) {
        ...
        if (hasParty() == false) {
          ...
          cm.sendOk(
            '请确保你的队伍人员最小等级在 ' +
              ...
              '之间. I see #b' +
              levelValid +
              '#k members are in the right level range. If this seems wrong, #blog out and log back in,#k or reform the party.'
          );
          cm.dispose();
          //} else if (checkPartyLevels() == false) {//判断队伍成员等级
          //   cm.sendOk("管理员 - 提示 \r\n\r\n队伍成员等级必须在#b" + minlvl + " - " + maxlvl + "#k之间并且必须在一张地图才能进入，请核对后在来找我。")
          //   cm.dispose();
        } else if (checkPartySize() == false) {
          //判断队伍成员人数
        ...
        if (
          cm.getPlayerCount(922010100) <= 0 &&
          cm.getPlayerCount(922010200) <= 0 &&
          cm.getPlayerCount(922010201) <= 0 &&
          cm.getPlayerCount(922010300) <= 0 &&
          cm.getPlayerCount(922010400) <= 0 &&
          cm.getPlayerCount(922010401) <= 0 &&
          cm.getPlayerCount(922010402) <= 0 &&
          cm.getPlayerCount(922010403) <= 0 &&
          cm.getPlayerCount(922010404) <= 0 &&
          cm.getPlayerCount(922010405) <= 0 &&
          cm.getPlayerCount(922010500) <= 0 &&
          cm.getPlayerCount(922010501) <= 0 &&
          cm.getPlayerCount(922010502) <= 0 &&
          cm.getPlayerCount(922010503) <= 0 &&
          cm.getPlayerCount(922010504) <= 0 &&
          cm.getPlayerCount(922010505) <= 0 &&
          cm.getPlayerCount(922010506) <= 0 &&
          cm.getPlayerCount(922010600) <= 0 &&
          cm.getPlayerCount(922010700) <= 0 &&
          cm.getPlayerCount(922010800) <= 0 &&
          cm.getPlayerCount(922010900) <= 0 &&
          cm.getPlayerCount(922011000) <= 0 &&
          cm.getPlayerCount(922011100) <= 0
        ) {
...
function getPartySize() {
  if (cm.getPlayer().getParty() == null) return 0;
  return cm.getPlayer().getParty().getMembers().size();
}

function isLeader() {
  if (cm.getParty() == null) return false;
  return cm.isLeader();
}

function checkPartySize() {
  var size = 0;
  if (cm.getPlayer().getParty() == null) size = 0;
  else size = cm.getPlayer().getParty().getMembers().size();
  if (size < minplayers || size > maxplayers) return false;
  return true;
}
...
function checkPartyLevels() {
  ...
  if (cm.getPlayer().getParty() == null) pass = false;
  else {
    for (var i = 0; i < party.size() && pass; i++) {
      if (party.get(i).getLevel() < minlvl || party.get(i).getLevel() > maxlvl || party.get(i).getMapId() != cm.getMapId()) pass = false;
    }
  }
  return pass;
}
...
function hasParty() {
  if (cm.getPlayer().getParty() == null) return false;
  return true;
}
...
function zdLog() {
  ...
  if (zd.getBossLog(Log) >= maxenter) return false;
  return true;
}
