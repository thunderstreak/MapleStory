...
function action(mode, type, selection) {
  if (mode == -1) {
    ...
    if (status == 0) cm.sendSimple('欢迎来到家族公馆，你现在想做什么呢?\r\n#b#L0#创建家族 #r1500W#k#l\r\n#L1#解散家族#l\r\n#L2#增加家族成员人数上限#l#k');
    else if (status == 1) {
      sel = selection;
      if (selection == 0) {
        if (cm.getChar().getGuildId() > 0) {
          ...
          cm.dispose();
        } else cm.sendYesNo('创建一个新的家族需要 #b1500W金币#k, 你确定继续创建一个新的家族吗？');
      } else if (selection == 1) {
        if (cm.getChar().getGuildId() <= 0 || cm.getChar().getGuildRank() != 1) {
          cm.sendOk('你还没有家族！\r\n你不是族长，因此你不能解散该家族.');
          cm.dispose();
        } else cm.sendYesNo('你确定真的要解散你的家族？当解散后你将不能恢复所有家族相关资料以及GP的数值，是否继续？');
      } else if (selection == 2) {
        if (cm.getChar().getGuildId() <= 0 || cm.getChar().getGuildRank() != 1) {
        ...
        else cm.sendYesNo('家族成员人数每增加 #b5#k 位需要支付 #b2500000 金币#k, 你确定要增加吗？');
      }
    } else if (status == 2) {
