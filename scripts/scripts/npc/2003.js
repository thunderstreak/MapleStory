...
function action(mode, type, selection) {
  ...
  if (status == 0) {
    ...
    if (selection == 0) {
      ...
      status = -1;
      cm.sendNext(
        '好的，这就是你移动的方式. 使用键盘上的 #b←左箭头, 和 →右箭头#k 就可以在平地和倾斜的道路上移动, 并按 键盘左下角的#bAlt#k键跳跃. 精选的鞋子可以提高你的移动速度和弹跳能力。看到地图里的发光处，那是传送门，走过去按键盘上的 #b↑上箭头#k .就可以进入传送门去往别的地图，按 ↓下箭头是趴下来。'
      );
    } else if (selection == 1) {
      ...
      status = -1;
      cm.sendNext(
        '你想成为一名#b战士#k? 恩, 那么我建议你去金银岛.勇士部落前往一个叫战士圣殿的地方。找NPC#r武术教练#k and see #bDances with Balrog#k. 他会教你成为真正的战士.额，一个很重要的事情；你必须至少10级才能成为一名战士！！'
      );
    } else if (selection == 7) {
      // What should I do to become a Bowman?
      status = -1;
      cm.sendNext(
        '你想成为一名#b弓箭手#k? 恩, 那么我建议你去金银岛.射手村前往一个叫弓箭手培训中心地方。找NPC#r赫丽娜#k. 他会教你成为真正的弓箭手.额，一个很重要的事情；你必须至少10级才能成为一名弓箭手！！'
      );
    } else if (selection == 8) {
      ...
      status = -1;
      cm.sendNext(
        '你想知道如何穿戴衣服？按I键打开背包，鼠标移动到你想穿戴的装备上面，双击它，就可以装备道具了。当然如果你发现自己穿不上这件装备，那么就是你的职业属性等不符合这件装备的设定，那么你就无法穿戴了。你也可以通过打开仓库对该装备进行存储，或打开商店对该装备进行卖出处理。也可以鼠标单击装备，然后把图标拖出装备窗口，再按一下鼠标左键，就可以把物品丢弃掉了。怎么样？会了吧！'
      );
      ...
      status = -1;
      cm.sendNext(
        '获得一份工作后，获得的特殊能力叫做技能。你将获得专门的技能，按键盘上的“K”键，可以查看你的技能列表，也可以给技能增加点数，强化技能的属性值。四转后，需要技能书，才能帮你加满技能哦~！'
      );
    ...
    // How can I pick up an item?
    cm.sendNextPrev(
      '但是要记住，如果你的背包满了，就无法捡取了。所以如果你有个道具不需要了，卖掉它，这样你就可以从中有所收获，一旦你的等级提高，了解的更多的时候，就会发现可以扩大背包，这样你就能获得并存放更多的道具了'
    );
  } else if (status == 7) {
    status = 0;
    cm.sendSimple(mainmenu);
    ...
    // When can I choose a job?
    cm.sendNextPrev(
      '等级不是唯一决定进步的因素，你还需要提高到一定的级别，增加以职业为基础的能力值。例如要成为一个战士，你的力量值必须超过35点，等等，你知道我在说什么吗？确保你知道你对自己的职业有个清晰的了解。'
    );
  } else if (status == 13) {
    status = 0;
    cm.sendSimple(mainmenu);
    ...
    // What should I do to become a Magician?
    cm.sendNextPrev(
      '哦，顺便说一下，不像别的职业，成为一个魔法师，你只需要在8级就可以转职了。为什么法师的入门标准低于其他职业呢？因为法师升级慢啊，前期没有法术配合，就是个小辣鸡，所以低于别的职业两级就能转职啦。哈哈。'
    );
  } else if (status == 21) {
    status = 0;
    cm.sendSimple(mainmenu);
