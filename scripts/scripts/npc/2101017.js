...
function start() {
  ...
  if (cm.getPlayer().getMapId() % 10 == 1) cm.sendSimple('你对我有一个请求吗?\r\n#b#L0# 给我 #t2270002# and #t2100067#.#l\r\n#L1# 我该做什么?#l\r\n#L2# 让我离开这里.#l');
  else
    cm.sendSimple(
      cm.getPlayer().getAriantRoomLeaderName(((cm.getPlayer().getMapId() / 100) % 10) - 1) == cm.getPlayer().getName()
        ? '你想开始比赛吗?#b\r\n#b#L3# 准备进入战场!!#l\r\n#L1# 我想踢另一个角色.#l\r\n#L2# 让我离开这里.#l'
        : '你想要什么?#b\r\n#L2# 让我离开这里.#l'
    );
}
...
function action(mode, type, selection) {
  ...
  if (cm.getPlayer().getMapId() % 10 == 1) {
    if (status == 0) {
      ...
      if (sel == 0) {
        if (cm.haveItem(2270002)) cm.sendNext('你已经拥有 #b#t2270002##k.');
        ...
        } else cm.sendNext('检查和查看是否使用库存是全');
        cm.dispose();
      } else if (sel == 1) {
        status = 1;
    ...
    else if (status == 4) cm.sendNextPrev('一件事. #r你可能不会使用宠物.#k理解?~!');
    else if (status == 5) cm.dispose();
  } else {
    ...
    if (status == 0) {
      ...
      if (sel == 1)
        if (cm.getPlayerCount(cm.getPlayer().getMapId()) > 1) {
          ...
          for (var i = 0; nextchar.hasNext(); i++) {
            ...
            if (
              !cm
                .getPlayer()
                .getAriantRoomLeaderName(((cm.getPlayer().getMapId() / 100) % 10) - 1)
                .equals(name)
            )
              text += '\r\n#b#L' + i + '#' + name + '#l';
          }
        ...
        else cm.sendYesNo('你确定你要离开吗?'); //No GMS like.
      } else if (sel == 3)
        if (cm.getPlayerCount(cm.getPlayer().getMapId()) > 0) cm.sendYesNo('房间都是一套，没有其他的角色可以加入这场战斗的竞技场。你想现在开始游戏吗?');
        else {
      ...
      if (sel == 1) {
        ...
        if (cm.getPlayer().getAriantRoomLeaderName(((cm.getPlayer().getMapId() / 100) % 10) - 1) != cm.getPlayer().getName()) cm.warp(980010000);
        else {
