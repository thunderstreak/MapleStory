  ...
  if (mode == -1) {
    ...
    if (status == 0) {
      ...
      if (cm.getPlayer().getLevel() < 70) {
        ...
        (cm.getPlayer().getBossLog('mrsbossrw') > 0 && cm.getPlayer().getBossLog('mrsbossrwa') > 0 && cm.getBossLog('SBOSSRW') < cm.getPlayer().getmrsbossrws()) ||
        cm.getBossLog('SBOSSRWA') < cm.getPlayer().getmrsbossrwas()
      ) {
        txt = '你好，这里是每日BOSS任务，在野外升级杀BOSS的同时也能完成任务。这种待遇很不错吧。\r\n\r\n';
        ...
        cm.getBossLog('SBOSSRW') >= cm.getPlayer().getmrsbossrws() &&
        cm.getBossLog('SBOSSRWA') >= cm.getPlayer().getmrsbossrwas() &&
        cm.getPlayer().getmrsbossrws() > 0 &&
        cm.getPlayer().getmrsbossrwas() > 0
      ) {
        txt = '你好，这里是每日BOSS任务，在野外升级杀BOSS的同时也能完成任务。这种待遇很不错吧。\r\n\r\n';
