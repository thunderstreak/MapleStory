...
function action(mode, type, selection) {
  ...
  if (status == 0) {
    ...
    cm.sendNextPrev(
      "Now, now, children. Don''t make this harder than it needs to be. Just walk towards me, nice and easy... Wait, you''re not one of the test subjects. You''re one of the townspeople, aren''t you?"
    );
  } else if (status == 3) {
    cm.sendNextPrevS("That''s right. I''m a resident of Edelstein, not a test subject. You can''t boss ME around.", 2);
  } else if (status == 4) {
    cm.sendNextPrev(
      "Oh my, oh my. I told them to make sure the townspeople kept their kids away from the mines... Alas, it''s too late now. I can''t allow you to tell anyone about this laboratory, so I guess you''ll just have to stay here and...help with the experiments. *snicker*"
    );
  } else if (status == 5) {
