...
function action(mode, type, selection) {
  ...
  if (status == 0) {
    ...
    if (selection == 0) {
      ...
      for (var i = 0; i < 3; i += 1)
        if (cm.getPlayerCount(980010100 + i * 100) > 0)
          if (cm.getPlayerCount(980010101 + i * 100) > 0) continue;
          else
            text +=
              '\r\n#L' +
              i +
          ...
          if (cm.getPlayer().getAriantRoomLeaderName(i) != '') cm.getPlayer().removeAriantRoom(i);
        }
      cm.sendSimple(text);
    } else if (selection == 1) {
      cm.sendNext(
        '阿里安特竞技场是一个激烈的战场，真正的战士将被清理！即使你是个胆小鬼，你也不要把你的眼睛放在上面！一个探险家能使阿列达喜爱的宝石最会被选为最好的宝物！很简单，是吧？\r\n - #e等级#n : 50或以上#r(推荐等级 : 50 - 80 )#k\r\n - #e时间限制#n : 8 分钟\r\n - #e队员#n : 2-6\r\n - #e物品#n :\r\n#i1113048:# 冠军戒指'
      );
      cm.dispose();
    } else if (selection == 2) {
      status = 9;
    ...
    var sel = selection;
    if (cm.getPlayer().getAriantRoomLeaderName(sel) != '' && empty[sel]) empty[sel] = false;
    ...
    // todo: code champion rings :c
    cm.dispose();
  } else if (status == 10) {
    cm.sendNextPrev(
      '让我告诉你最简单的规则。最伟大的冠军 #b灵魂宝石#k将被选为最佳的冠军。当然，如果你赢得了一场比赛的话，你会得到更高的赞美 #b无数冠军#k.\r\n\r\n(#b当比赛结束时，你的排名将会被你所拥有的灵魂宝石所决定。此外，如果更多的参与者继续，您将获得更多的奖励.)#k'
    );
  } else if (status == 11) {
    cm.sendNextPrev('即使你不够坚强，也不要担心。如果你能做 #b至少 15#k 灵魂的宝石，没有人敢否认的事实，你是一个伟大的战士.\r\n\r\n(如果你做 #b至少 15 灵魂宝石, 你将获得平均报酬.)#k');
  } else if (status == 12) {
    cm.sendNextPrev(
      '如果你赚的多 15?当然，我们会对这种特殊的冠军奖励更多的奖励！这并不意味着你会得到 #r无限量的奖励#k, 虽然。如果你做 #b30#k 宝石，你会得到的 #r最佳报酬#k.\r\n\r\n(使 #b30 灵魂宝石获得最高质量奖励.)#k'
    );
  } else if (status == 13) {
    cm.sendNextPrev(
      '如果你不做至少15颗宝石，那就意味着你不会得到任何奖励? 不，那不可能是这样！我们美丽的女王阿列达吩咐我们给予一定的奖励 #b冠军谁甚至失败了至少 15#k 宝石。在这种情况下，你会得到 #r较少的奖励#k. 有什么抱怨吗？如果你不喜欢它，训练你的技能和执行一个竞技场比赛中！\r\n\r\n(如果你做 #b少于15 灵魂宝石，你将获得低质量的奖励.)#k'
    );
  } else if (status == 14) {
    cm.sendNextPrev(
      '当然，一个臭名昭著的失败者不值得被对待，以及其他人。即使 #b6 灵魂宝石#k 都太多了 #r你要做的#k, 那么，这仅仅意味着你没有达到标准。不管怎样，你几乎不受 #r任何奖励#k 为比赛中的比赛。所以，试着得到至少6个或更多的宝石.\r\n\r\n(如果你做 #b5 或更少的灵魂宝石，你将获得几乎任何奖励.)#k'
    );
  } else if (status == 15) {
