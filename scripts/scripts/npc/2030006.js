...
var qChars = new Array(
  '问题1: 下面中冒险岛最初遇见的NPC是谁？#微微安#库克斯#希娜# 村长塔塔曼#3', //5
  ...
  '问题1: 下面中怪物和怪物所掉的战利品连接正确的是哪一个?#蓝蜗牛-蘑菇壳#蝙蝠-蝙蝠翅膀#白雪人-企鹅的嘴巴#火狗-火焰之羽#2'
); //6
var qItems = new Array(
  '问题2: 下面中下面中在勇士部落没有的NPC的谁?#大姐大#易德#酷男孩#飞天猪#2', //6
  ...
  '问题2: 下面中在神秘岛看不见的怪物是什么?#蓝蘑菇#石头人#黑鄂鱼#巫婆#3'
); //1
var qMobs = new Array(
  '问题3: 下面中射手村的玛亚为只俩了自己的病要求的药是什么?#特殊的药#解毒药#止咳药水#奇怪的药#4', //2
  ...
  '问题3: 下面中要求等级最高的任务是什么?#阿尔卡斯特和黑暗水晶#好无聊#帮忙写作业#木妖好可怕#1'
); //4
var qQuests = new Array(
  '问题4: 下面中药水和药水效果连接不正确的是什么?#解毒药－解除中毒#活力神水－恢复MP300#黄昏之露－恢复MP5000#清晨之露－恢复3000#4', //1
...
var qTowns = new Array(
  '问题5: 下面中药水和药水效果连接不恰当的是什么?#鸡蛋-HP50#烤肉-HP100#蓝色药水-MP100#披萨-HP400恢复#4', //4
  ...
  '问题5: 下面中能飞行的怪物是什么?#狼人#蘑菇王#企鹅#@巫婆#4'
); //3
var correctAnswer = 0;
...
function action(mode, type, selection) {
  if (mode == -1) cm.dispose();
    ...
    if (status == 1) cm.sendNextPrev('#h #,如果你给我一个#v4005004#我会让你试试回答5个问题，让他们都正确，你将获得#v4031058# #b智慧项链#k.');
    else if (status == 2) {
      ...
      if (selection == correctAnswer) cm.sendOk('#h # 你是正确的.\n准备好进入下一个问题了吗 ?');
      else {
        cm.sendOk('你得到一个问题答案是错误的!\r\n请你认真检查自己的知识。');
        cm.dispose();
      }
    } else if (status == 4) cm.sendSimple('现在让我们来进入下一个环节.\r\n\r\n' + getQuestion(qItems[Math.floor(Math.random() * qItems.length)]));
    else if (status == 5) {
      if (selection == correctAnswer) cm.sendOk('#h # 你是正确的.\n准备好进入下一个问题了吗 ?');
      else {
        cm.sendOk('有一题做错了!\r\n(这些都是很容易的)\r\n请认真思考答案');
      ...
      if (selection == correctAnswer) cm.sendOk('#h # 你是正确的。让我们进入下一个关卡 ?');
      else {
        cm.sendOk('看来你没有回答正确啊');
        cm.dispose();
      }
    } else if (status == 8) cm.sendSimple('下面让我们进入这一道选择.\r\n\r\n' + getQuestion(qQuests[Math.floor(Math.random() * qQuests.length)]));
    else if (status == 9) {
