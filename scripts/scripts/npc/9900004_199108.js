    ...
    if (status == 0) {
      ...
      if (selection == 0) {
        ...
        if (cm.getPlayer().getjf() < 100) {
          ...
          cm.dispose();
        } else cm.sendOk('抱歉，您尚未赞助，或者已经领取了该档礼包~.');
        cm.dispose();
      } else if (selection == 1) {
        //if (!cm.beibao(1, 1)) {//前面的1对应装备第一栏，也就是装备 后面就是格数
        //   cm.sendOk("装备栏空余不足3个空格。请清理背包以免影响获得物品！");//判断不等于，就提示对话
