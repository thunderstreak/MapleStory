...
function action(mode, type, selection) {
  ...
  if (status == 0) {
    cm.sendNext(
      "Scrolls are always regarded as the important aspect of Mapler's equipment upgrade process. Are you one of those who strive for achieving perfection in the crafted material?#b\r\n#L0#Create a 65% Scroll#l\r\n#L1#Create a 15% scroll#l#k"
    );
  } else if (status == 1) {
    firstSel = selection;
    cm.sendYesNo(
      'Oh, hey there. You seems to be more interested than I do. I can always upgrade your scrolls with a little fee and some materials. Are you sure you want to do that? #bThere is a 10% chance that the scroll might be destroyed in the process of the creation.#k'
    );
  } else if (status == 2) {
    cm.sendNext(
      "Now here's the ingredients for resurrection of the ancient black magic scroll. \n\r #i4011006# #t4011006# x1 \n\r #i4021007# #t4021007# x1 \n\r #i4021008# #t4021008# x1 \n\r Mesos 500,000 \n\r\n\r Lastly, it will also require the 10% scroll."
    );
  } else if (status == 3) {
    var message = "Here's the list of scrolls that are within my powers to be created : \n\r";
    if (firstSel == 0) {
      for (var i = 0; i < scroll_15.length; i++) {
        if ((scroll_15[i] != 2045206 && scroll_15[i] != 2045306) || cm.isGMS()) {
          //TODO JUMP
