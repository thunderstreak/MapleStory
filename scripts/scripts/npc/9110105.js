...
function action(mode, type, selection) {
  if (mode == 1) {
    ...
    if (status == 0) {
      cm.sendOk(
        "...see? What lies ahead is a treacherous path, one that's known to chew up and spilt out everyone that dare to go there. If I were you, I'd turn around and leave right now with my life in tact."
      );
    }
    cm.dispose();
    return;
