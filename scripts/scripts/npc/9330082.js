...
function action(mode, type, selection) {
  ...
  if (status == 0) {
    ...
    switch (selection) {
      ...
      case 2:
        cm.sendGetNumber(
          '你要用多少 ' + (selection == 1 ? '#i3993002#' : '#i3993002#') + ' 兑换BOSS 点数? \r\n现在比值 1 个#t3993002# : #b1万点数#k\r\n',
          cm.itemQuantity(selection == 1 ? 3993002 : 3993002),
          1,
          9999
        );
        break;
...
function chooseItem(index) {
  ...
  for (var i = 0; i < items[index].length; i++)
    choice +=
      ...
      items[index][i][0] +
      '#)' +
      (items[index][i][2] > 0 ? ' X #r#e' + items[index][i][2] + '#n#b个' : '') +
      (items[index][i][3] > 0 ? ' ...期限 #r#e' + items[index][i][3] + '#n#b天' : '') +
      '#l';
  choice += '\r\n ';
