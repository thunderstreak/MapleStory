...
function action(mode, type, selection) {
  ...
  if (mode != 1) {
    ...
    if (status == 0) {
      ...
      var selStr =
        'Total Wins: ' +
        cm.getPlayer().getTotalWins() +
        ', Total Losses: ' +
        cm.getPlayer().getTotalLosses() +
        ', Current Wins: ' +
        cm.getPlayer().getIntNoRecord(122400) +
        ...
        if (battlers[i] != null) {
          try {
            selStr +=
              ...
              battlers[i].getName() +
              ' (#o' +
              battlers[i].getMonsterId() +
              '#) Level ' +
              battlers[i].getLevel() +
              ' ' +
              battlers[i].getGenderString() +
              ', HP #B' +
              battlers[i].getHPPercent() +
              '# - ' +
              battlers[i].getCurrentHP() +
              '/' +
              battlers[i].calcHP() +
              ', Status: ' +
              battlers[i].getStatusString() +
              '#l\r\n';
          } catch (e) {
            cm.sendOk('Error: ' + e);
            cm.dispose();
      ...
      if (battlers[0] == null) {
        ...
        cm.sendOk(
          'You have obtained your first minion. You can head down to the Internet Cafe to train or get other minions. You can also use @challenge <player name> to challenge someone to a match.'
        );
        cm.dispose();
        return;
      }
