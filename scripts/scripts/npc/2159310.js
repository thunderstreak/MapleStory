...
function action(mode, type, selection) {
  ...
  if (status == 0) {
    ...
    cm.sendNextNoESC(
      "Whenever Orchid talks, our meetings grind to a halt! As for the Heroes, I'm sure #h0# has a plan to deal with them. I'm sure these pathetic 'Heroes' will be no match for him.",
      2159308
    );
  } else if (status == 11) {
    cm.sendPlayerToNpc(
      'Unlike most foes, the Heroes fight for others, not themselves... they are special, because they protect the world. That makes them dangerous. Also, I only stunned the Goddess. The Black Mage was the one to defeat her.'
    );
  } else if (status == 12) {
    ...
    cm.sendDirectionInfo('Effect/Direction6.img/effect/tuto/balloonMsg0/10');
    cm.sendNextNoESC(
      'Enough! Accept that #h0# stunned the Goddess, allowing for our victory. Therefore his contribution was the most important. Besides, you are credited for blinding the Goddess. What more do you want?'
    );
  } else if (status == 17) {
