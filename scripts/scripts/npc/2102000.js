var gm;

function start() {
  status = -1;
  gm = cm.getEventManager('Geenie');
  action(1, 0, 0);
}

function action(mode, type, selection) {
  if (mode == -1) {
    cm.dispose();
    return;
  } else {
    status++;
    if (mode == 0) {
      cm.sendNext('你一定要在这儿办点事，对吗？?');
      cm.dispose();
      return;
    }
    if (status == 0) {
      if (gm == null) {
        cm.sendNext('事件脚本错误, 请通知管理员');
        cm.dispose();
      } else if (gm.getProperty('entry').equals('true')) {
        cm.sendYesNo('This will not be a short flight, so if you need to take care of some things, I suggest you do that first before getting on board. Do you still wish to board the ship?');
      } else if (gm.getProperty('entry').equals('false') && gm.getProperty('docked').equals('true')) {
        cm.sendNext("The ship is getting ready for takeoff. I'm sorry, but you'll have to get on the next ride. The ride schedule is available through the usher at the ticketing booth.");
        cm.dispose();
      } else {
        cm.sendNext(
          'We will begin boarding 5 minutes before the takeoff. Please be patient and wait for a few minutes. Be aware that the ship will take off on time, and we stop receiving tickets 1 minute before that, so please make sure to be here on time.'
        );
        cm.dispose();
      }
    } else if (status == 1) {
      cm.warp(260000110);
      cm.dispose();
    }
  }
}
