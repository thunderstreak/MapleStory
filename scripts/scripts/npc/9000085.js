...
function action(mode, type, selection) {
  ...
  if (mode != 1) {
    ...
    if (status == 0) {
      ...
      var selStr =
        'Seen: #r#e' +
        cm.getPlayer().getMonsterBook().getSeen() +
        '#n#k, Caught: #b#e' +
        cm.getPlayer().getMonsterBook().getCaught() +
      ...
      if (byName && cm.getText() != null && cm.getText().length() > 0) {
        var found = false;
        var selStr =
          'Seen: #r#e' + cm.getPlayer().getMonsterBook().getSeen() + '#n#k, Caught: #b#e' + cm.getPlayer().getMonsterBook().getCaught() + '#n#k\r\nWhich entry would you like to see?\r\n\r\n#b';
        for (var i = 0; i < pokeEntries.size(); i++) {
