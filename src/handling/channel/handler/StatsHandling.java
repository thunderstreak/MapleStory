...
public class StatsHandling {
    ...
    public static void DistributeAP(final SeekableLittleEndianAccessor slea, final MapleClient c,
    ...
    public static void DistributeSP(final int skillid, final MapleClient c, final MapleCharacter chr) {
        ...
        if (skill.hasRequiredSkill() && chr.getSkillLevel(SkillFactory.getSkill(skill.getRequiredSkillId())) < skill
        ...
        if (skill.isInvisible() && chr.getSkillLevel(skill) == 0
                && ((skill.isFourthJob() && chr.getMasterLevel(skill) == 0)
    ...
    public static void AutoAssignAP(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        ...
        for (int count = slea.readInt(), i = 0; i < count; ++i) {
            ...
            if (chr.getRemainingAp() >= updatenumber) {
                switch (update) {
                    ...
                    default: {
                        c.getSession().write(MaplePacketCreator.updatePlayerStats(MaplePacketCreator.EMPTY_STATUPDATE,
                                true, chr.getJob()));
                        return;
                    }
                }
                ...
                StatsHandling.log.info("[h4x] Player {} is distributing AP to {} without having any", chr.getName(),
                        update);
            }
        }
        statupdate.add(new Pair<MapleStat, Integer>(MapleStat.AVAILABLEAP, (int) chr.getRemainingAp()));
