...
public class InventoryHandler {
    ...
    public static boolean UseRewardItem(final byte slot, final int itemId, final MapleClient c,
                        ...
                        for (final StructRewardItem reward : rewards.getRight()) {
                            if (reward.prob > 0 && Randomizer.nextInt(rewards.getLeft()) < reward.prob) {
                                if (GameConstants.getInventoryType(reward.itemid) == MapleInventoryType.EQUIP) {
                                    ...
                                    if (reward.period > 0L) {
                                        item.setExpiration(
                                                System.currentTimeMillis() + reward.period * 60L * 60L * 10L);
                                    }
    ...
    public static void UseReturnScroll(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
    ...
    public static boolean UseUpgradeScroll(final byte slot, final byte dst, final byte ws, final MapleClient c,
            final MapleCharacter chr) {
    ...
    public static boolean UseUpgradeScroll(final byte slot, final byte dst, final byte ws, final MapleClient c,
        ...
        if (!GameConstants.isSpecialScroll(scroll.getItemId()) && !GameConstants.isCleanSlate(scroll.getItemId())
            ...
            if (toScroll.getUpgradeSlots() >= 1 || toScroll.getEnhance() >= 100 || vegas > 0
                ...
                && (toScroll.getState() >= 1 || (toScroll.getLevel() == 0 && toScroll.getUpgradeSlots() == 0)
        ...
        if (legendarySpirit && vegas == 0 && chr.getSkillLevel(SkillFactory.getSkill(1003)) <= 0
            ...
            AutobanManager.getInstance().addPoints(c, 50, 120000L,
    ...
    public static void UseCatchItem(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        c.getPlayer().updateTick(slea.readInt());
        final byte slot = (byte) slea.readShort();
        final int itemid = slea.readInt();
        ...
        if (toUse != null && toUse.getQuantity() > 0 && toUse.getItemId() == itemid && mob != null) {
            switch (itemid) {
                ...
                case 2270002: {
                    ...
                    if (mob.getHp() <= mob.getMobMaxHp() / 2L) {
                        c.getPlayer().getMap()
                                .broadcastMessage(MaplePacketCreator.catchMonster(mob.getId(), itemid, (byte) 1));
                        map.killMonster(mob, chr, true, false, (byte) 0);
                        MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, itemid, 1, false, false);
                        c.getPlayer().setAPQScore(c.getPlayer().getAPQScore() + 1);
                        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator
                                .updateAriantPQRanking(c.getPlayer().getName(), c.getPlayer().getAPQScore(), false));
                        break;
                    }
                    c.getPlayer().getMap()
                            .broadcastMessage(MaplePacketCreator.catchMonster(mob.getId(), itemid, (byte) 0));
    ...
    public static void UseMountFood(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        c.getPlayer().updateTick(slea.readInt());
        final byte slot = (byte) slea.readShort();
        final int itemid = slea.readInt();
    ...
    public static void UseSummonBag(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        if (!chr.isAlive()) {
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
    ...
    public static void UseTreasureChest(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        ...
        if (useCash && chr.getCSPoints(2) < price) {
            ...
            if (rareness > 0 || reward > 0) {
                World.Broadcast.broadcastMessage(MaplePacketCreator.getGachaponMega(c.getPlayer().getName(),
                        " : 从" + box + "中获得{" + ii.getName(item.getItemId()) + "}！大家一起恭喜他（她）吧！！！！", item, rareness,
                        c.getChannel()));
            }
        } else {
            ...
            switch (itemId) {
                ...
                case 5041000: {
                    ...
                    if (victim != null && !victim.isGM() && c.getPlayer().getEventInstance() == null
                        ...
                        if (!FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit())
                                && !FieldLimitType.VipRock.check(
                                        c.getChannelServer().getMapFactory().getMap(victim.getMapId()).getFieldLimit())
                                && (itemId == 5041000
                ...
                case 5050000: {
                    ...
                    if (used) {
                        ...
                        c.getSession()
                                .write(MaplePacketCreator.updatePlayerStats(statupdate, true, c.getPlayer().getJob()));
                        break;
                    }
                    break;
                }
                ...
                case 5050004: {
                    ...
                    if (c.getPlayer().getSkillLevel(skillSPTo) + 1 <= skillSPTo.getMaxLevel()
                        ...
                        c.getPlayer().changeSkillLevel(skillSPFrom,
                ...
                case 5080002:
                case 5080003: {
                    final MapleLove love = new MapleLove(c.getPlayer(), c.getPlayer().getPosition(),
                ...
                case 5570000: {
                    slea.readInt();
                    final Equip item2 = (Equip) c.getPlayer().getInventory(MapleInventoryType.EQUIP)
                            .getItem((byte) slea.readInt());
                    if (item2 == null) {
                        break;
                    }
                    if (GameConstants.canHammer(item2.getItemId())
                            && MapleItemInformationProvider.getInstance().getSlots(item2.getItemId()) > 0
                            && item2.getViciousHammer() <= 2) {
                ...
                case 5077000: {
                    ...
                    if (c.getPlayer().isPlayer()) {
                        World.Broadcast.broadcastSmega(
                                MaplePacketCreator.tripleSmega(messages, ear3, c.getChannel()).getBytes());
                ...
                case 5073000: {
                    ...
                    if (c.getPlayer().isPlayer()) {
                        World.Broadcast.broadcastSmega(
                                MaplePacketCreator.serverNotice(11, c.getChannel(), sb.toString(), ear).getBytes());
                        System.out.println("[玩家廣播頻道 " + c.getPlayer().getName() + "] : " + message);
                    } else if (c.getPlayer().isGM()) {
                        World.Broadcast.broadcastSmega(
                                MaplePacketCreator.serverNotice(11, c.getChannel(), sb.toString(), ear).getBytes());
                ...
                case 5074000: {
                    ...
                    if (c.getPlayer().isPlayer()) {
                        World.Broadcast.broadcastSmega(
                                MaplePacketCreator.serverNotice(12, c.getChannel(), sb.toString(), ear).getBytes());
                        System.out.println("[玩家廣播頻道 " + c.getPlayer().getName() + "] : " + message);
                    } else if (c.getPlayer().isGM()) {
                        World.Broadcast.broadcastSmega(
                                MaplePacketCreator.serverNotice(12, c.getChannel(), sb.toString(), ear).getBytes());
                ...
                case 5072000: {
                    ...
                    if (c.getPlayer().isPlayer()) {
                        World.Broadcast.broadcastSmega(
                                MaplePacketCreator.serverNotice(3, c.getChannel(), sb.toString(), ear).getBytes());
                        System.out.println("[玩家廣播頻道 " + c.getPlayer().getName() + "] : " + message);
                    } else if (c.getPlayer().isGM()) {
                        World.Broadcast.broadcastSmega(
                                MaplePacketCreator.serverNotice(3, c.getChannel(), sb.toString(), ear).getBytes());
                ...
                case 5076000: {
                    ...
                    if (c.getPlayer().isPlayer()) {
                        World.Broadcast.broadcastSmega(
                                MaplePacketCreator.itemMegaphone(sb.toString(), ear, c.getChannel(), item3).getBytes());
                        System.out.println("[玩家廣播頻道 " + c.getPlayer().getName() + "] : " + message);
                    } else if (c.getPlayer().isGM()) {
                        World.Broadcast.broadcastSmega(
                                MaplePacketCreator.itemMegaphone(sb.toString(), ear, c.getChannel(), item3).getBytes());
                ...
                case 5075005: {
                    ...
                    MapleCharacter victim2 = (tvType == 1 || tvType == 4) ? null
                    ...
                    World.Broadcast.broadcastSmega(MaplePacketCreator
                            .serverNotice(3, c.getChannel(), c.getPlayer().getName() + " : " + message3, ear2)
                            .getBytes());
                    break;
                }
                case 5090000:
                ...
                case 5190008: {
                    ...
                    if (zz != null && !zz.check(pet.getFlags())) {
                        ...
                        c.getSession()
                                .write(PetPacket.updatePet(pet, c.getPlayer().getInventory(MapleInventoryType.CASH)
                ...
                case 5170000: {
                    ...
                    pet2.setName(nName);
                    c.getSession().write(PetPacket.updatePet(pet2, c.getPlayer().getInventory(MapleInventoryType.CASH)
                            .getItem((byte) pet2.getInventoryPosition()), true));
                    c.getSession().write(MaplePacketCreator.enableActions());
                ...
                case 5240028: {
                    ...
                    c.getSession().write(PetPacket.updatePet(pet2,
                            c.getPlayer().getInventory(MapleInventoryType.CASH).getItem(pet2.getInventoryPosition()),
                            true));
                ...
                case 5281000:
                case 5281001: {
                    final Rectangle bounds = new Rectangle((int) c.getPlayer().getPosition().getX(),
                ...
                case 5390006: {
                    ...
                    if (c.getPlayer().isPlayer()) {
                        World.Broadcast.broadcastSmega(MaplePacketCreator
                                .getAvatarMega(c.getPlayer(), c.getChannel(), itemId, text, ear2).getBytes());
                ...
                case 5500001:
                case 5500002: {
                    final IItem item4 = c.getPlayer().getInventory(MapleInventoryType.EQUIPPED)
                            .getItem(slea.readShort());
                    final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
                    final int days2 = 20;
                    if (item4 != null && !GameConstants.isAccessory(item4.getItemId()) && item4.getExpiration() > -1L
                            && !ii.isCash(item4.getItemId()) && System.currentTimeMillis()
                                    + 8640000000L > item4.getExpiration() + days2 * 24 * 60 * 60 * 1000L) {
                        boolean change = true;
                ...
                default: {
                    switch (itemId / 10000) {
                        case 512: {
                            ...
                            final String msg = ii2.getMsg(itemId).replaceFirst("%s", c.getPlayer().getName())
                                    .replaceFirst("%s", slea.readMapleAsciiString());
    ...
    public static void Pickup_Player(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        if (c.getPlayer().getPlayerShop() != null || c.getPlayer().getConversation() > 0
                || c.getPlayer().getTrade() != null) {
            return;
            ...
            if (Distance > 2500.0) {
                ...
                World.Broadcast
                        .broadcastGMMessage(MaplePacketCreator
                                .serverNotice(6, "[GM消息] " + chr.getName() + " ID: " + chr.getId() + " (等级 "
                    ...
                    for (final MapleCharacter i : toGive) {
                        i.gainMeso(
                                mapitem.getMeso() / toGive.size()
                                        + (i.getStat().hasPartyBonus ? ((int) (mapitem.getMeso() / 20.0)) : 0),
                                true, true);
                    }
                } else {
    ...
    public static void Pickup_Pet(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        if (chr == null) {
            return;
        }
        ...
        if (mapitem.isPlayerDrop() && mapitem.getDropType() == 0 && mapitem.getOwner() == chr.getId()
                && mapitem.getMeso() != 0) {
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        }
        ...
        if (Distance > 10000.0 && (mapitem.getMeso() > 0 || mapitem.getItemId() != 4001025)) {
            ...
            World.Broadcast
                    .broadcastGMMessage(
                            MaplePacketCreator
                                    .serverNotice(6,
                                            "[GM消息] " + chr.getName() + " ID: " + chr.getId() + " (等级 " + chr.getLevel()
                                                    + ") 全屏宠吸。地图ID: " + chr.getMapId() + " 范围: " + Distance)
                                    .getBytes());
        ...
        if (mapitem.getMeso() > 0) {
            if (chr.getParty() != null && mapitem.getOwner() != chr.getId()) {
                ...
                for (final MapleCharacter i : toGive) {
                    i.gainMeso(splitMeso / toGive.size()
                            + (i.getStat().hasPartyBonus ? ((int) (mapitem.getMeso() / 20.0)) : 0), true);
                }
            ...
            MapleInventoryManipulator.pet_addFromDrop(c, mapitem.getItem(), true,
                    mapitem.getDropper() instanceof MapleMonster);
            removeItem_Pet(chr, mapitem, petz);
        }
    }
    ...
    public static boolean UseSkillBook(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        slea.skip(4);
        final byte slot = (byte) slea.readShort();
        final int itemId = slea.readInt();
