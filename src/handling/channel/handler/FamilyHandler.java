...
public class FamilyHandler {
    public static void RequestFamily(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        final MapleCharacter chr = c.getChannelServer().getPlayerStorage()
    ...
    public static void UseFamily(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        boolean success = c.getPlayer().getFamilyId() > 0 && c.getPlayer().canUseFamilyBuff(entry)
        ...
        switch (type) {
            case 0: {
                ...
                if (victim.getFamilyId() == c.getPlayer().getFamilyId()
                        && !FieldLimitType.VipRock.check(victim.getMap().getFieldLimit())
                        && victim.getId() != c.getPlayer().getId()) {
            ...
            case 1: {
                ...
                } else if (victim.getFamilyId() == c.getPlayer().getFamilyId()
                        && !FieldLimitType.VipRock.check(victim.getMap().getFieldLimit())
                        && victim.getId() != c.getPlayer().getId()) {
                    victim.getClient().getSession().write(FamilyPacket.familySummonRequest(c.getPlayer().getName(),
    ...
    public static void FamilyOperation(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        final MapleCharacter addChr = c.getChannelServer().getPlayerStorage()
                .getCharacterByName(slea.readMapleAsciiString());
        if (addChr == null) {
    ...
    public static void FamilySummon(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        final MapleCharacter tt = c.getChannelServer().getPlayerStorage()
                .getCharacterByName(slea.readMapleAsciiString());
    ...
    public static void DeleteJunior(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        MapleCharacterUtil.sendNote(other.getName(), c.getPlayer().getName(), c.getPlayer().getName() + " 我做人失败 解散了家族",
                0);
    ...
    public static void DeleteSenior(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        c.getPlayer().saveFamilyStatus();
        MapleCharacterUtil.sendNote(mgc.getName(), c.getPlayer().getName(), c.getPlayer().getName() + " 我展翅高飞了 离开你的家族",
                0);
            ...
            if (accepted) {
                ...
                if (inviter.getFamilyId() > 0 && World.Family.getFamily(inviter.getFamilyId()) != null) {
                    ...
                    c.getPlayer().setFamily((old <= 0) ? inviter.getFamilyId() : old, inviter.getId(),
                            (oldj1 <= 0) ? 0 : oldj1, (oldj2 <= 0) ? 0 : oldj2);
                    ...
                    if (id > 0) {
                        ...
                        c.getPlayer().setFamily(id, inviter.getId(), (oldj1 <= 0) ? 0 : oldj1,
                                (oldj2 <= 0) ? 0 : oldj2);
