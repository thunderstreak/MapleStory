...
public class PlayerHandler {
    ...
    public static void TrockAddMap(final SeekableLittleEndianAccessor slea, final MapleClient c,
    ...
    public static void TakeDamage(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        chr.updateTick(slea.readInt());
        ...
        if (damage == -1) {
            ...
            AutobanManager.getInstance().addPoints(c, 1000, 60000L,
        ...
        if (damage > 0) {
            ...
            if (type != -2 && type != -3 && type != -4) {
                final int bouncedam_ = ((Randomizer.nextInt(100) < chr.getStat().DAMreflect_rate)
                        ...
                        : 0)
                        + ((type == -1 && chr.getBuffedValue(MapleBuffStat.伤害反击) != null)
                                ? chr.getBuffedValue(MapleBuffStat.伤害反击)
                                : 0)
            ...
            if (zhanshenkangya != null && damage > 0) {
                ...
                if (attacker != null) {
                    ...
                    final int kangyashanghai = (int) (kangya.getEffect(chr.getSkillLevel(21101003)).getDamage() / 100.0
                            * damage);
            ...
            if (attacker != null && bouncedam_A != null && damage > 0) {
                ...
                final int bouncedamage2 = (int) (kangya2.getEffect(chr.getSkillLevel(21101003)).getDamage() / 100.0
                        * damage);
    ...
    public static void SpecialMove(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
            ...
            default: {
                ...
                if (effect.is时空门()) {
                    ...
                    if (mountid != 0 && mountid != GameConstants.getMountItem(skill.getId()) && !c.getPlayer().isGM()
                            && c.getPlayer().getBuffedValue(MapleBuffStat.骑兽技能) == null
                            && c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-118)) == null
    ...
    public static void closeRangeAttack(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr, final boolean energy) {
        if (chr == null || (energy && chr.getBuffedValue(MapleBuffStat.能量获得) == null
                && chr.getBuffedValue(MapleBuffStat.战神抗压) == null && !GameConstants.isKOC(chr.getJob()))) {
            return;
        }
        ...
        if (隐身 != null) {
            ...
            if ((chr.getJob() != 400 || chr.getJob() != 410 || chr.getJob() != 411 || chr.getJob() != 412)
                    && attack.skill != 4221001 && attack.skill != 4221007 && attack.skill != 0) {
        ...
        ISkill skill = null;
        if (attack.skill == 21100004 || attack.skill == 21100005 || attack.skill == 21110004 || attack.skill == 21120006
                || attack.skill == 21120007) {
        ...
        chr.checkFollow();
        // 检测攻击速度和攻击异常
        chr.getCheatTracker().checkAttack(attack.skill, attack.lastAttackTickCount);
        chr.getMap().broadcastMessage(chr,
    ...
    public static void MagicDamage(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        if (chr == null) {
            return;
        }
    ...
    public static void Heal(final SeekableLittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        if (chr.canHP() && healHP != 0) {
            ...
            if (healHP > check_hp * 2 && healHP > 20) {
                chr.getCheatTracker().registerOffense(CheatingOffense.回复过多HP,
                        String.valueOf(healHP) + " 服务器:" + check_hp);
            }
            chr.addHP(healHP);
        }
        if (chr.canMP() && healMP != 0) {
            if (healMP > check_mp * 2 && healMP > 20) {
                chr.getCheatTracker().registerOffense(CheatingOffense.回复过多MP,
                        String.valueOf(healMP) + "服务器:" + check_mp);
            }
            chr.addMP(healMP);
        }
    }
    ...
    public static void ChangeMap(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        if (chr == null || chr.getMap() == null) {
        ...
        if (slea.available() != 0L) {
            ...
            if (targetid != -1 && !chr.isAlive()) {
                ...
                final MapleMap to = ChannelServer.getInstance(c.getChannel()).getMapFactory().getMap(targetid);
                if (c.getPlayer().isGM() || (chr.getMapId() == 0 && to.getId() == 10000)
                        || (chr.getMapId() == 2010000 && to.getId() == 104000000)
                        || (chr.getMapId() == 1020100 && to.getId() == 1020000)
                        || (chr.getMapId() == 1020200 && to.getId() == 1020000)
                        || (chr.getMapId() == 1020300 && to.getId() == 1020000)
                        || (chr.getMapId() == 1020400 && to.getId() == 1020000)
                        || (chr.getMapId() == 1020500 && to.getId() == 1020000)
                        || (chr.getMapId() == 914090010 && to.getId() == 914090011)
                        || (chr.getMapId() == 914090011 && to.getId() == 914090012)
                        || (chr.getMapId() == 914090012 && to.getId() == 914090013)
                        || (chr.getMapId() == 914090013 && to.getId() == 140090000)
                        || (chr.getMapId() == 914090100 && to.getId() == 140000000)) {
    ...
    public static void ChangeMap33(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        if (chr == null || chr.getMap() == null) {
            NPCScriptManager.getInstance().dispose(c);
            c.getSession().write(MaplePacketCreator.enableActions());
        ...
        if (slea.available() != 0L) {
            ...
            if (targetid != -1 && !chr.isAlive()) {
                ...
                if (!wheel) {
                    ...
                    c.getSession().write(
                            MTSCSPacket.useWheel(chr.getInventory(MapleInventoryType.CASH).countById(5510000) - 1));
                    chr.getStat().setHp(chr.getStat().getMaxHp() / 100 * 40);
                    MapleInventoryManipulator.removeById(c, MapleInventoryType.CASH, 5510000, 1, true, false);
                    final MapleMap to = chr.getMap();
                    chr.changeMap(to, to.getPortal(0));
    ...
    public static final void ChangeMap20(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        if (chr == null) {
    ...
    public static void InnerPortal(final SeekableLittleEndianAccessor slea, final MapleClient c,
            final MapleCharacter chr) {
        if (chr == null) {
            return;
        }
