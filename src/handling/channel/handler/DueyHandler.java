...
public class DueyHandler {
    public static void DueyOperation(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (operation) {
                                    ...
                                    if (!ii.isDropRestricted(item.getItemId())
                                        ...
                                        if (addItemToDB(item, amount, mesos, c.getPlayer().getName(), accid,
                                            ...
                                            if (GameConstants.is飞镖道具(item.getItemId())
                                                ...
                                                MapleInventoryManipulator.removeFromSlot(c, inv, (byte) itemPos, amount,
                                                        true, false);
                                            }
            ...
            case 5: {
                ...
                if (dp.getItem() != null && !MapleInventoryManipulator.checkSpace(c, dp.getItem().getItemId(),
                        dp.getItem().getQuantity(), dp.getItem().getOwner())) {
    ...
    private static boolean addItemToDB(final IItem item, final int quantity, final int mesos, final String sName,
            final int recipientID, final boolean isOn) {
        final Connection con = DatabaseConnection.getConnection();
        try {
            final PreparedStatement ps = con.prepareStatement(
                    "INSERT INTO dueypackages (RecieverId, SenderName, Mesos, TimeStamp, Checked, Type) VALUES (?, ?, ?, ?, ?, ?)",
            ...
            if (rs.next()) {
                ItemLoader.DUEY.saveItems(Collections.singletonList(
    ...
    private static void removeItemFromDB(final int packageid, final int charid) {
        ...
        try {
            final PreparedStatement ps = con
                    .prepareStatement("DELETE FROM dueypackages WHERE PackageId = ? and RecieverId = ?");
            ps.setInt(1, packageid);
            ps.setInt(2, charid);
            ps.executeUpdate();
            ps.close();
