...
public class MapleGuild implements Serializable {
    ...
    public static void meso(final MapleClient c, final int npcid) {
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement(
                    "SELECT `name`,`meso`,`vip`, `level`FROM characters ORDER BY `meso` DESC LIMIT 100");
            final ResultSet rs = ps.executeQuery();
    ...
    public static void 战斗力排行(final MapleClient c, final int npcid) {
        try {
            ...
            try (final PreparedStatement ps = con.prepareStatement(
                    "select `name`, ((`level` * 15) + CASE WHEN fame < 0 THEN 0 ELSE fame END + (maxhp / 50) + (maxmp / 50) + str + dex + luk + `int`) AS `data`, `level`, meso from characters order by `data` desc LIMIT 100")) {
                rs = ps.executeQuery();
    ...
    public static void 破攻排行(final MapleClient c, final int npcid) {
        try {
            ...
            ResultSet rs;
            try (final PreparedStatement ps = con.prepareStatement(
                    "select `name`, `PGMaxDamage` AS `data`, `level`, meso from characters order by `data` desc LIMIT 100")) {
                rs = ps.executeQuery();
                c.getSession().write(MaplePacketCreator.showCustomRanks(npcid, rs));
            }
            rs.close();
    ...
    public static void 总在线时间排行(final MapleClient c, final int npcid) {
        try {
            ...
            ResultSet rs;
            try (final PreparedStatement ps = con.prepareStatement(
                    "select `name`, totalOnlineTime AS `data`, `level`, meso from characters order by `data` desc LIMIT 100")) {
                rs = ps.executeQuery();
                c.getSession().write(MaplePacketCreator.showCustomRanks(npcid, rs));
            }
            rs.close();
    ...
    public static void 豆豆排行(final MapleClient c, final int npcid) {
        try {
            ...
            ResultSet rs;
            try (final PreparedStatement ps = con.prepareStatement(
                    "select `name`, beans AS `data`, `level`, meso from characters order by `data` desc LIMIT 100")) {
                rs = ps.executeQuery();
                c.getSession().write(MaplePacketCreator.showCustomRanks(npcid, rs));
            }
            rs.close();
    ...
    public static void MapleMSpvpdeaths(final MapleClient c, final int npcid) {
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement(
                    "SELECT `name`, `pvpdeaths`, `str`, `dex`, `int`, `luk` FROM characters ORDER BY `pvpdeaths` DESC LIMIT 20");
            final ResultSet rs = ps.executeQuery();
    ...
    public static void MapleMSpvpkills(final MapleClient c, final int npcid) {
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement(
                    "SELECT `name`, `pvpkills`, `str`, `dex`, `int`, `luk` FROM characters ORDER BY `pvpkills` WHERE gm < 1  DESC LIMIT 100");
            final ResultSet rs = ps.executeQuery();
    ...
    public static void 人气排行(final MapleClient c, final int npcid) {
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement(
                    "SELECT `name`, `fame`, `level`, `meso` FROM characters ORDER BY `fame` DESC LIMIT 100");
            final ResultSet rs = ps.executeQuery();
    ...
    public static MapleGuildResponse sendInvite(final MapleClient c, final String targetName) {
        ...
        mc.getClient().getSession().write(MaplePacketCreator.guildInvite(c.getPlayer().getGuildId(),
                c.getPlayer().getName(), c.getPlayer().getLevel(), c.getPlayer().getJob()));
        return null;
    }

    public static void setOfflineGuildStatus(final int guildid, final byte guildrank, final byte alliancerank,
            final int cid) {
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement(
    ...
    public MapleGuild(final int guildid) {
        ...
        try {
            ...
            do {
                ...
                this.members.add(new MapleGuildCharacter(rs.getInt("id"), rs.getShort("level"), rs.getString("name"),
                        (byte) (-1), rs.getInt("job"), rs.getByte("guildrank"), guildid, rs.getByte("alliancerank"),
                        false));
            } while (rs.next());
            rs.close();
            ps.close();
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                final MapleBBSThread thread = new MapleBBSThread(rs.getInt("localthreadid"), rs.getString("name"),
                ...
                while (rse.next()) {
                    thread.replies.put(thread.replies.size(), new MapleBBSThread.MapleBBSReply(thread.replies.size(),
                            rse.getInt("postercid"), rse.getString("content"), rse.getLong("timestamp")));
                }
                rse.close();
                pse.close();
            ...
            if (!bDisband) {
                ...
                for (final MapleBBSThread bb : this.bbs.values()) {
                    ...
                    if (!rs.next()) {
                        ...
                        final PreparedStatement pse = con.prepareStatement(
    ...
    public static void 杀怪排行榜(final MapleClient c, final int npcid) {
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement(
                    "SELECT `name`, `shaguai`, `str`, `dex`, `int`, `luk` FROM characters ORDER BY `shaguai` DESC LIMIT 100");
            final ResultSet rs = ps.executeQuery();
    ...
    public void setOnline(final int cid, final boolean online, final int channel) {
        ...
        if (bBroadcast) {
            ...
            if (this.allianceid > 0) {
                World.Alliance.sendGuild(MaplePacketCreator.allianceMemberOnline(this.allianceid, this.id, cid, online),
                        this.id, this.allianceid);
            }
        }
        this.bDirty = true;
        this.init = true;
        ...
        try {
            ...
            for (int i = this.members.size() - 1; i >= 0; --i) {
                if (this.members.get(i).getGuildRank() < 5
                        || this.members.get(i).getName().compareTo(mgc.getName()) < 0) {
    ...
    public void memberLevelJobUpdate(final MapleGuildCharacter mgc) {
        for (final MapleGuildCharacter member : this.members) {
            if (member.getId() == mgc.getId()) {
                ...
                if (this.allianceid > 0) {
                    World.Alliance.sendGuild(MaplePacketCreator.updateAlliance(mgc, this.allianceid), this.id,
                            this.allianceid);
                    break;
                }
                break;
    ...
    public int addBBSThread(final String title, final String text, final int icon, final boolean bNotice,
            final int posterID) {
    ...
    public void editBBSThread(final int localthreadid, final String title, final String text, final int icon,
        ...
        if (thread != null && (thread.ownerID == posterID || guildRank <= 2)) {
            this.bbs.put(localthreadid, new MapleBBSThread(localthreadid, title, text, System.currentTimeMillis(),
                    this.id, thread.ownerID, icon));
        }
    }
    ...
    public void addBBSReply(final int localthreadid, final String text, final int posterID) {
        final MapleBBSThread thread = this.bbs.get(localthreadid);
        if (thread != null) {
            thread.replies.put(thread.replies.size(), new MapleBBSThread.MapleBBSReply(thread.replies.size(), posterID,
