...
public class MapleFamily implements Serializable {
    ...
    public static void setOfflineFamilyStatus(final int familyid, final int seniorid, final int junior1,
            final int junior2, final int currentrep, final int totalrep, final int cid) {
        try {
            final Connection con = DatabaseConnection.getConnection();
    ...
    public MapleFamily(final int fid) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                this.members.put(rs.getInt("id"),
                        new MapleFamilyCharacter(rs.getInt("id"), rs.getShort("level"), rs.getString("name"), -1,
            ...
            if (this.leadername == null || this.members.size() < 2) {
                System.err.println("Leader " + this.leaderid + " isn't in family " + this.id
            ...
            for (final MapleFamilyCharacter mfc : this.members.values()) {
                if (mfc.getJunior1() > 0
                        && (this.getMFC(mfc.getJunior1()) == null || mfc.getId() == mfc.getJunior1())) {
                    mfc.setJunior1(0);
                }
                if (mfc.getJunior2() > 0 && (this.getMFC(mfc.getJunior2()) == null || mfc.getId() == mfc.getJunior2()
                        || mfc.getJunior1() == mfc.getJunior2())) {
                    mfc.setJunior2(0);
                }
                if (mfc.getSeniorId() > 0
                        && (this.getMFC(mfc.getSeniorId()) == null || mfc.getId() == mfc.getSeniorId())) {
                    mfc.setSeniorId(0);
                }
            ...
            if (!bDisband) {
                if (this.changed) {
                    final PreparedStatement ps = con
                            .prepareStatement("UPDATE families SET notice = ? WHERE familyid = ?");
    ...
    public void setOnline(final int cid, final boolean online, final int channel) {
        ...
        if (mgc != null && mgc.getFamilyId() == this.id) {
            if (mgc.isOnline() != online) {
                this.broadcast(FamilyPacket.familyLoggedIn(online, mgc.getName()), cid,
                        (mgc.getId() == this.leaderid) ? null : mgc.getPedigree());
            }
    ...
    public int setRep(final int cid, int addrep, final int oldLevel) {
        ...
        if (mgc != null && mgc.getFamilyId() == this.id) {
            ...
            if (mgc.isOnline()) {
                ...
                World.Family.setFamily(this.id, mgc.getSeniorId(), mgc.getJunior1(), mgc.getJunior2(),
                        mgc.getCurrentRep() + addrep, mgc.getTotalRep() + addrep, mgc.getId());
            } else {
                setOfflineFamilyStatus(this.id, mgc.getSeniorId(), mgc.getJunior1(), mgc.getJunior2(),
                        mgc.getCurrentRep() + addrep, mgc.getTotalRep() + addrep, mgc.getId());
            }
    ...
    public MapleFamilyCharacter addFamilyMemberInfo(final MapleCharacter mc, final int seniorid, final int junior1,
            final int junior2) {
    ...
    public void memberLevelJobUpdate(final MapleCharacter mgc) {
        ...
        if (member != null) {
            ...
            if (old_level != mgc.getLevel()) {
                this.broadcast(MaplePacketCreator.sendLevelup(true, mgc.getLevel(), mgc.getName()), mgc.getId(),
                        (mgc.getId() == this.leaderid) ? null : member.getPedigree());
            }
            if (old_job != mgc.getJob()) {
                this.broadcast(MaplePacketCreator.sendJobup(true, mgc.getJob(), mgc.getName()), mgc.getId(),
                        (mgc.getId() == this.leaderid) ? null : member.getPedigree());
            }
        }
    }
    ...
    public boolean splitFamily(final int splitId, final MapleFamilyCharacter def) {
        ...
        try {
            ...
            for (final MapleFamilyCharacter mgc2 : all) {
                if (mgc2.isOnline()) {
                    World.Family.setFamily(newId, mgc2.getSeniorId(), mgc2.getJunior1(), mgc2.getJunior2(),
                            mgc2.getCurrentRep(), mgc2.getTotalRep(), mgc2.getId());
                }
