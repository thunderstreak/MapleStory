...
public class MapleFamilyBuff {
    ...
    static {
        ...
        name = new String[] { "直接移动到学院成员身边", "直接召唤学院成员", "我的爆率 1.5倍(15分钟)", "我的经验值 1.5倍(15分钟)", "学院成员的团结(30分钟)",
        ...
        for (int i = 0; i < 2; ++i) {
            MapleFamilyBuff.buffEntries.add(new MapleFamilyBuffEntry(i, MapleFamilyBuff.name[i],
                    MapleFamilyBuff.desc[i], 1, MapleFamilyBuff.rep[i], MapleFamilyBuff.type[i], 190000 + i,
                    MapleFamilyBuff.duration[i], MapleFamilyBuff.effect[i]));
        }
    }
    ...
    public static class MapleFamilyBuffEntry {
        ...
        public MapleFamilyBuffEntry(final int index, final String name, final String desc, final int count,
                final int rep, final int type, final int questID, final int duration, final int effect) {
            this.name = name;
            this.desc = desc;
            this.count = count;
            this.rep = rep;
        ...
        public void applyTo(final MapleCharacter chr) {
            chr.getClient().getSession()
                    .write(MaplePacketCreator.giveBuff(-this.getEffectId(), this.duration * 60000, this.effects, null));
            ...
            final MapleStatEffect.CancelEffectAction cancelAction = new MapleStatEffect.CancelEffectAction(chr, eff,
                    starttime);
            final ScheduledFuture<?> schedule = Timer.BuffTimer.getInstance().schedule(cancelAction,
                    starttime + this.duration * 60000 - starttime);
