...
public class MaplePvpStats implements Serializable {
    ...
    public MaplePvpStats(final int watk, final int matk, final int wdef, final int mdef, final int acc, final int avoid,
            final int wdef_rate, final int mdef_rate, final int ignore_def, final int damage_rate,
            final int ignore_damage) {
        this.watk = watk;
        this.matk = matk;
        this.wdef = wdef;
        this.mdef = mdef;
    ...
    public static MaplePvpStats loadOrCreateFromDB(final int accountId) {
        ...
        try {
            ...
            if (rs.next()) {
                ret = new MaplePvpStats(rs.getInt("watk"), rs.getInt("matk"), rs.getInt("wdef"), rs.getInt("mdef"),
                        rs.getInt("acc"), rs.getInt("avoid"), rs.getInt("wdef_rate"), rs.getInt("mdef_rate"),
                        rs.getInt("ignore_def"), rs.getInt("damage_rate"), rs.getInt("ignore_damage"));
            } else {
                final PreparedStatement psu = con.prepareStatement(
                        "INSERT INTO pvpstats (accountid, watk, matk, wdef, mdef, acc, avoid, wdef_rate, mdef_rate, ignore_def, damage_rate, ignore_damage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public void saveToDb(final int accountId) {
        try {
            final PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement(
                    "UPDATE pvpstats SET watk = ?, matk = ?, wdef = ?, mdef = ?, acc = ?, avoid = ?, wdef_rate = ?, mdef_rate = ?, ignore_def = ?, damage_rate = ?, ignore_damage = ? WHERE accountId = ?");
            ps.setInt(1, accountId);
