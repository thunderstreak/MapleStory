                ...
                if (!MapleLieDetector.this.isPassed() && MapleLieDetector.this.chr != null)
                    if (MapleLieDetector.this.attempt >= 3) {
                        ...
                        if (search_chr != null && search_chr.getId() != MapleLieDetector.this.chr.getId()) {
                            search_chr.dropMessage(5,
                                    MapleLieDetector.this.chr.getName() + " 没用通过测谎仪的检测，恭喜你获得7000的金币.");
                            search_chr.gainMeso(7000, true);
                        }
                        MapleLieDetector.this.end();
                        MapleLieDetector.this.chr.getClient().getSession()
                                .write(MaplePacketCreator.LieDetectorResponse((byte) 8, (byte) 4));
                        MapleMap map = MapleLieDetector.this.chr.getClient().getChannelServer().getMapFactory()
                                .getMap(180000001);
                        MapleLieDetector.this.chr.getQuestNAdd(MapleQuest.getInstance(123456))
                                .setCustomData(String.valueOf(1800));
