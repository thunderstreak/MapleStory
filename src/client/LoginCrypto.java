...
public class LoginCrypto {
    ...
    public static String rand_s(final String in) {
        ...
        for (int i = 0; i < LoginCrypto.extralength; ++i) {
            sb.append(LoginCrypto.rand.nextBoolean()
                    ? LoginCrypto.Alphabet[LoginCrypto.rand.nextInt(LoginCrypto.Alphabet.length)]
                    : LoginCrypto.Number[LoginCrypto.rand.nextInt(LoginCrypto.Number.length)]);
        }
