...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    private static MerchItemPackage loadItemFrom_Database(final int charid, final int accountid) {
        ...
        try {
            final PreparedStatement ps = con
                    .prepareStatement("SELECT * from hiredmerch where characterid = ? OR accountid = ?");
            ps.setInt(1, charid);
            ...
            ps.close();
            rs.close();
            final Map<Integer, Pair<IItem, MapleInventoryType>> items = ItemLoader.HIRED_MERCHANT.loadItems(false,
                    charid);
            if (items != null) {
    ...
    public NPCConversationManager(final MapleClient c, final int npc, final int questid, final byte type,
            final Invocable iv, final int wh) {
        super(c);
        this.lastMsg = -1;
    ...
    public void sendSimple(final String text, final int speaker) {
        ...
        this.getClient().getSession()
                .write(MaplePacketCreator.getNPCTalk(this.npc, (byte) 4, text, "", (byte) speaker));
        this.lastMsg = 4;
    }
    ...
    public int gainGachaponItem(final int id, final int quantity) {
        return this.gainGachaponItem(id, quantity,
                this.c.getPlayer().getMap().getStreetName() + " - " + this.c.getPlayer().getMap().getMapName());
    }
        ...
        try {
            ...
            if (rareness > 0) {
                World.Broadcast.broadcastMessage(
                        MaplePacketCreator.getGachaponMega("[" + msg + "] " + this.c.getPlayer().getName(),
                                " : 恭喜获得道具!", item, rareness, this.getPlayer().getClient().getChannel()).getBytes());
            }
        ...
        try {
            ...
            if (概率 > 0) {
                World.Broadcast.broadcastMessage(MaplePacketCreator
                        .getGachaponMega("[" + msg + "] " + this.c.getPlayer().getName(), " : 从抽奖中获得!大家恭喜他（她）吧！！！",
                                item, (byte) 0, this.getPlayer().getClient().getChannel())
                        .getBytes());
            }
            return item.getItemId();
        } catch (Exception e) {
    ...
    public void warpPartyWithExp(final int mapId, final int exp) {
        ...
        for (final MaplePartyCharacter chr : this.getPlayer().getParty().getMembers()) {
            final MapleCharacter curChar = this.c.getChannelServer().getPlayerStorage()
    ...
    public boolean registerSquad(final String type, final int minutes, final String startText) {
        if (this.c.getChannelServer().getMapleSquad(type) == null) {
            final MapleSquad squad = new MapleSquad(this.c.getChannel(), type, this.c.getPlayer(), minutes * 60 * 1000,
                    startText);
    ...
    public void displayGuildRanks() {
        this.c.getSession()
                .write(MaplePacketCreator.showGuildRanks(this.npc, MapleGuildRanking.getInstance().getGuildRank()));
    }
    ...
    public void killAllMonsters() {
        ...
        for (final MapleMapObject monstermo : map.getMapObjectsInRange(this.c.getPlayer().getPosition(), range,
    ...
    public void dc() {
        final MapleCharacter victim = this.c.getChannelServer().getPlayerStorage()
                .getCharacterByName(this.c.getPlayer().getName());
    ...
    public void openMerchantItemStore1() {
        final MerchItemPackage pack = loadItemFrom_Database(this.c.getPlayer().getId(),
                this.c.getPlayer().getAccountID());
    ...
    public boolean addFromDrop(final Object statsSel) {
        if (statsSel instanceof IItem) {
            ...
            return MapleInventoryManipulator.checkSpace(this.getClient(), it.getItemId(), it.getQuantity(),
                    it.getOwner()) && MapleInventoryManipulator.addFromDrop(this.getClient(), it, false);
        }
        return false;
    }

    public boolean replaceItem(final int slot, final int invType, final Object statsSel, final int offset,
            final String type) {
        return this.replaceItem(slot, invType, statsSel, offset, type, false);
    }

    public boolean replaceItem(final int slot, final int invType, final Object statsSel, final int offset,
            final String type, final boolean takeSlot) {
    ...
    public boolean createAlliance(final String alliancename) {
        ...
        final MapleCharacter otherChar = this.c.getChannelServer().getPlayerStorage()
    ...
    public void doWeddingEffect2(final Object ch) {
        ...
        this.getMap().broadcastMessage(MaplePacketCreator.yellowChat(this.getPlayer().getName() + ", 你愿意娶 "
                + chr.getName() + " 为妻吗？无论她将来是富有还是贫穷、或无论她将来身体健康或不适，你都愿意和她永远在一起吗？"));
        Timer.CloneTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (chr == null || NPCConversationManager.this.getPlayer() == null) {
                    ...
                    NPCConversationManager.this.getMap()
                            .broadcastMessage(MaplePacketCreator.yellowChat(
                                    chr.getName() + ", 你愿意嫁给 " + NPCConversationManager.this.getPlayer().getName()
                                            + " 吗？无论他将来是富有还是贫穷、或无论他将来身体健康或不适，你都愿意和他永远在一起吗?"));
                }
            }
        }, 10000L);
        Timer.CloneTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (chr == null || NPCConversationManager.this.getPlayer() == null) {
                    if (NPCConversationManager.this.getPlayer() != null) {
                        NPCConversationManager.this.setQuestRecord(NPCConversationManager.this.getPlayer(), 160001,
                                "3");
                        NPCConversationManager.this.setQuestRecord(NPCConversationManager.this.getPlayer(), 160002,
                                "0");
                    } else if (chr != null) {
                    ...
                    NPCConversationManager.this
                            .sendNPCText(
                                    "好，我以圣灵、圣父、圣子的名义宣布：" + NPCConversationManager.this.getPlayer().getName() + " 和 "
                                            + chr.getName() + ", 结为夫妻。 希望你们在!"
                                            + chr.getClient().getChannelServer().getServerName() + " 游戏中玩的愉快!",
                                    9201002);
                    NPCConversationManager.this.getMap().startExtendedMapEffect(
                            "You may now kiss the bride, " + NPCConversationManager.this.getPlayer().getName() + "!",
                            5120006);
                    if (chr.getGuildId() > 0) {
                    ...
                    if (NPCConversationManager.this.getPlayer().getGuildId() > 0) {
                        World.Guild.guildPacket(NPCConversationManager.this.getPlayer().getGuildId(), MaplePacketCreator
                                .sendMarriage(false, NPCConversationManager.this.getPlayer().getName()));
                    }
                    if (NPCConversationManager.this.getPlayer().getFamilyId() > 0) {
                        World.Family.familyPacket(NPCConversationManager.this.getPlayer().getFamilyId(),
                                MaplePacketCreator.sendMarriage(true, chr.getName()),
                                NPCConversationManager.this.getPlayer().getId());
                    }
                }
            }
        }, 20000L);
                    ...
                    if (ii.itemExists(itemId)) {
                        ...
                        if (itemId == 0) {
                            ...
                            namez = de.Minimum * this.getClient().getChannelServer().getMesoRate() + " - "
                                    + de.Maximum * this.getClient().getChannelServer().getMesoRate() + " 的金币";
                        }
                        ch = de.chance * rate;
                        name.append(num + 1).append(") #v").append(itemId).append("#").append(namez).append(" - ")
                                .append(((ch >= 999999) ? 1000000 : ch) / 10000.0).append("%的爆率. ")
    ...
    public String checkMapDrop() {
        List<MonsterGlobalDropEntry> ranks = new ArrayList(
                MapleMonsterInformationProvider.getInstance().getGlobalDrop());
        ...
        if (ranks != null && ranks.size() > 0) {
            ...
            for (int i = 0; i < ranks.size(); i++) {
                ...
                if (de.continent < 0 || (de.continent < 10 && mapid / 100000000 == de.continent)
                    ...
                    if (getPlayer().isAdmin()) {
                        name.append(num + 1).append(") #v").append(itemId).append("#").append(names).append(" - ")
                        ...
                        name.append(num + 1).append(") #v").append(itemId).append("#").append(names)
                                .append((de.questid > 0 && MapleQuest.getInstance(de.questid).getName().length() > 0)
                                        ? ("需要接受任务: " + MapleQuest.getInstance(de.questid).getName())
                                        : "")
                                .append("\r\n");
    ...
    public void 设置签到奖励领取状态(final int slot) {
        try {
            ...
            try (final PreparedStatement ps = con.prepareStatement(
                    "UPDATE accounts SET qiandaolb =qiandaolb+ " + slot + " WHERE id = " + cid + "")) {
                ps.executeUpdate();
            }
        } catch (SQLException ex) {
            ex.getStackTrace();
    ...
    public void 设置礼包领取状态(final int slot) {
        try {
            ...
            try (final PreparedStatement ps = con
                    .prepareStatement("UPDATE accounts SET lb =lb+ " + slot + " WHERE id = " + cid + "")) {
                ps.executeUpdate();
            }
        } catch (SQLException ex) {
            ex.getStackTrace();
    ...
    public void setzb(final int slot) {
        try {
            ...
            final Connection con = DatabaseConnection.getConnection();
            try (final PreparedStatement ps = con
                    .prepareStatement("UPDATE accounts SET money =money+ " + slot + " WHERE id = " + cid + "")) {
                ps.executeUpdate();
            }
        } catch (SQLException ex) {
            ex.getStackTrace();
    ...
    public void setmoneyb(final int slot) {
        try {
            ...
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con
                    .prepareStatement("UPDATE accounts SET moneyb =moneyb+ " + slot + " WHERE id = " + cid + "");
            ps.executeUpdate();
            ps.close();
        } catch (SQLException ex) {
            ex.getStackTrace();
    ...
    public void warpBack(final int mid, final int retmap, final int time) {
        ...
        Timer.EventTimer.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                final MapleMap warpMap = NPCConversationManager.this.c.getChannelServer().getMapFactory()
                        .getMap(retmap);
    ...
    public void warpMapWithClock(final int mid, final int seconds) {
        ...
        Timer.MapTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (NPCConversationManager.this.c.getPlayer() != null) {
                    for (final MapleCharacter chr : NPCConversationManager.this.c.getPlayer().getMap()
    ...
    public int 给全服发物品(final int 物品ID, final int 数量, final int 力量, final int 敏捷, final int 智力, final int 运气,
            final int HP, final int MP, final int 可加卷次数, final String 制作人名字, final int 给予时间, final String 是否可以交易,
            final int 攻击力, final int 魔法力, final int 物理防御, final int 魔法防御) {
        int count = 0;
        try {
            final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            ...
            for (final ChannelServer cserv1 : ChannelServer.getAllInstances()) {
                for (final MapleCharacter mch : cserv1.getPlayerStorage().getAllCharacters()) {
                    if (数量 >= 0) {
                        ...
                        if ((type.equals(MapleInventoryType.EQUIP) && !GameConstants.isThrowingStar(物品ID)
                                && !GameConstants.isBullet(物品ID))
                        ...
                        MapleInventoryManipulator.removeById(mch.getClient(), GameConstants.getInventoryType(物品ID),
                                物品ID, -数量, true, false);
                    }
    ...
    public int 给当前地图发物品(final int 物品ID, final int 数量, final int 力量, final int 敏捷, final int 智力, final int 运气,
            final int HP, final int MP, final int 可加卷次数, final String 制作人名字, final int 给予时间, final String 是否可以交易,
            final int 攻击力, final int 魔法力, final int 物理防御, final int 魔法防御) {
        int count = 0;
        final int mapId = this.c.getPlayer().getMapId();
    ...
    public int 给当前频道发物品(final int 物品ID, final int 数量, final int 力量, final int 敏捷, final int 智力, final int 运气,
            final int HP, final int MP, final int 可加卷次数, final String 制作人名字, final int 给予时间, final String 是否可以交易,
            final int 攻击力, final int 魔法力, final int 物理防御, final int 魔法防御) {
        int count = 0;
        final int chlId = this.c.getPlayer().getMap().getChannel();
    ...
    public boolean 判断当前地图是否已禁用此脚本(final int scriptId) {
        try {
            ...
            final Connection con = DatabaseConnection.getConnection();
            try (final PreparedStatement ps = con
                    .prepareStatement("SELECT COUNT(*) as DATA FROM 禁用脚本地图 WHERE scriptId = ? AND mapId = ?")) {
    ...
    public int 认证主播() {
        try {
            final Connection con = DatabaseConnection.getConnection();
            try (final PreparedStatement ps = con
                    .prepareStatement("SELECT COUNT(*) as DATA FROM 认证主播 WHERE name = ?")) {
                ps.setString(1, this.getName());
                try (final ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        return rs.getInt("DATA");
    ...
    public int 给指定地图发物品(int 地图ID, final int 物品ID, final int 数量, final int 力量, final int 敏捷, final int 智力, final int 运气,
            final int HP, final int MP, final int 可加卷次数, final String 制作人名字, final int 给予时间, final String 是否可以交易,
            final int 攻击力, final int 魔法力, final int 物理防御, final int 魔法防御) {
        int count = 0;
        if (地图ID < 1) {
                    ...
                    if (mmo != null) {
                        ...
                        if (数量 >= 0) {
                            ...
                            if ((type.equals(MapleInventoryType.EQUIP) && !GameConstants.isThrowingStar(物品ID)
                                ...
                                MapleInventoryManipulator.addById(chr.getClient(), 物品ID, (short) 数量, "", null, 给予时间,
                                        (byte) 0);
                            }
                        } else {
                            MapleInventoryManipulator.removeById(chr.getClient(), GameConstants.getInventoryType(物品ID),
                                    物品ID, -数量, true, false);
                        }
            ...
            for (final MapleCharacter victim : cserv.getPlayerStorage().getAllCharacters()) {
                if (victim.getId() != this.c.getPlayer().getId()) {
                    victim.getMap().broadcastMessage(MaplePacketCreator.getChatText(victim.getId(),
                            StringUtil.joinStringFrom(内容, 1), victim.isGM(), 0));
                }
            }
        }
    }
    ...
    public static int 获取最高玩家在线() {
        ...
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con
                    .prepareStatement("SELECT MAX(totalOnlineTime) as DATA FROM characters WHERE gm = 0");
            try (final ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    data = rs.getInt("DATA");
                }
    ...
    public static String 获取最高在线玩家名字() {
        ...
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement(
                    "SELECT `name`, `totalOnlineTime` FROM characters WHERE gm = 0 ORDER BY `totalOnlineTime` DESC LIMIT 1");
            try (final ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    name = rs.getString("name");
    ...
    public int 获取自己等级排名() {
        ...
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement(
                    "SELECT rank FROM (SELECT @rownum := @rownum + 1 AS rank, `name`, `level`, `id` FROM characters, (SELECT @rownum := 0) r WHERE gm = 0 ORDER BY `level` DESC) AS T1 WHERE `id` = ?");
    ...
    public void 进入商城1() {
        try {
            ...
            this.c.getSession().write(MaplePacketCreator.getChannelChange(InetAddress.getByName(socket[0]),
        ...
        switch (lx) {
            case 1: {
                World.Broadcast.broadcastSmega(
                        MaplePacketCreator.serverNotice(11, this.c.getChannel(), "[全服公告] : " + msg).getBytes());
                break;
            }
            case 2: {
                World.Broadcast.broadcastSmega(
                        MaplePacketCreator.serverNotice(12, this.c.getChannel(), "[全服公告] : " + msg).getBytes());
                break;
            }
            case 3: {
                World.Broadcast.broadcastSmega(
                        MaplePacketCreator.serverNotice(3, this.c.getChannel(), "[全服公告] : " + msg).getBytes());
                break;
            }
            case 4: {
                World.Broadcast.broadcastSmega(
                        MaplePacketCreator.serverNotice(9, this.c.getChannel(), "[全服公告] : " + msg).getBytes());
                break;
            }
            case 5: {
                World.Broadcast.broadcastSmega(
                        MaplePacketCreator.serverNotice(6, this.c.getChannel(), "[全服公告] : " + msg).getBytes());
                break;
            }
            case 6: {
                World.Broadcast.broadcastSmega(
                        MaplePacketCreator.serverNotice(18, this.c.getChannel(), "[全服公告] : " + msg).getBytes());
                break;
            }
            case 7: {
                World.Broadcast.broadcastSmega(
                        MaplePacketCreator.serverNotice(10, this.c.getChannel(), "[全服公告] : " + msg).getBytes());
                break;
            }
        }
    }
    ...
    public void 刷新地图() {
        ...
        final int mapid = this.c.getPlayer().getMapId();
        final MapleMap map = custMap ? this.c.getPlayer().getClient().getChannelServer().getMapFactory().getMap(mapid)
                : this.c.getPlayer().getMap();
    ...
    public void 组队征集喇叭(final int lx, final String msg) {
        switch (lx) {
            case 1: {
                World.Broadcast.broadcastSmega(
                        MaplePacketCreator.serverNotice(11, this.c.getChannel(), "[组队征集令] : " + msg).getBytes());
                break;
            }
            case 2: {
                World.Broadcast.broadcastSmega(
                        MaplePacketCreator.serverNotice(12, this.c.getChannel(), "[组队征集令]: " + msg).getBytes());
                break;
            }
            case 3: {
                World.Broadcast.broadcastSmega(
                        MaplePacketCreator.serverNotice(3, this.c.getChannel(), "[组队征集令]: " + msg).getBytes());
                break;
            }
        }
    }
    ...
    public void deleteItem(final int inventorytype) {
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con
                    .prepareStatement("Select * from inventoryitems where characterid=? and inventorytype=?");
            ps.setInt(1, this.getPlayer().getId());
            ...
            while (re.next()) {
                MapleInventoryManipulator.removeById(this.getC(), type, re.getInt("itemid"), re.getInt("quantity"),
                        true, true);
            }
            re.close();
            ps.close();
    ...
    public void Startqmdb() throws InterruptedException, SQLException {
        for (int ii = 0; ii <= 20; ++ii) {
            ...
            if (iterator.hasNext()) {
                ...
                for (final MapleCharacter mch : cserv1.getPlayerStorage().getAllCharacters()) {
                    mch.getClient().getSession().write(MaplePacketCreator.sendHint(
                            "#b===========全民冒险岛==========#k\r\n==============================#r\r\n#b========全民夺宝活动开始=======#k\r\n==============================#r\r\n#b===========随机抽取中==========#k\r\n◆正在随机抽选中奖的幸运玩家◆\r\n#b===========幸运玩家===========#r\r\n"
                                    + mch.全民夺宝2(A),
                            200, 200));
                    if (ii == 20) {
                        mch.getClient().getSession().write(
                                MaplePacketCreator.sendHint("#e#r★★★★★全民夺宝★★★★★\r\n中奖玩家：" + mch.全民夺宝2(A), 200, 200));
