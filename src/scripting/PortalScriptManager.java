...
public class PortalScriptManager {
    ...
    private PortalScript getPortalScript(final MapleClient c, final String scriptName) {
        ...
        final File scriptFile = new File(
                scriptsPath + "scripts" + File.separator + "portal" + File.separator + scriptName + ".js");
    ...
    static {
        ...
        // Portal 脚本同样依赖 importPackage，因此强制使用 Nashorn 并开启 mozilla 兼容层
        ...
        if (engine != null) {
            ...
            System.out.println("[ScriptEngine] Portal 引擎: " + engineName);
            if (engineName != null && engineName.toLowerCase().contains("nashorn")) {
                try {
                    engine.eval("load('nashorn:mozilla_compat.js');");
                } catch (Exception e) {
                    System.err.println("加载 mozilla_compat.js 失败: " + e);
                }
            }
            sef = engine.getFactory();
        } else {
            throw new IllegalStateException("无法获取任意 JavaScript ScriptEngine，请检查依赖");
        }
    }
}
