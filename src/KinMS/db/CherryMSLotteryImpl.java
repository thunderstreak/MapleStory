    ...
    public void drawalottery() {
        ...
        if (drawchars != null) {
            for (MapleCharacter chr : drawchars) {
                ...
                if (charType == 2) {
                    toucount2++;
                    if (this.zjNum == 1 || this.zjNum == 3 || (this.zjNum == 5 && charNum == 1) || this.zjNum == 2
                            || this.zjNum == 4 || (this.zjNum == 6 && charNum == 2)) {
                        charZhuNX *= Integer.parseInt(ServerProperties.getProperty("RoyMS.赌博A赔率", "2"));
                        charZhuNX -= charZhuNX * Integer.parseInt(ServerProperties.getProperty("RoyMS.赌博手续费", "5"))
                                / 100;
                        ...
                        zhongcount2++;
                        zjpeople++;
                        getChannelServer().broadcastPacket(MaplePacketCreator.serverNotice(6,
                                "[赌博系统]" + Integer.parseInt(ServerProperties.getProperty("RoyMS.赌博A赔率", "2")) + "倍中奖名单:"
                                        + zjNames6 + "！恭喜他(她)吧"));
                    } else {
                        chr.dropMessage(1, "本期号码：【" + this.zjNum + "】\r\n对不起您没有中奖，请继续努力");
                    }
                ...
                if (charType == 3) {
                    toucount3++;
                    if ((this.zjNum > 4 && charNum > 4) || (this.zjNum < 3 && charNum < 3)
                            || (this.zjNum >= 3 && this.zjNum <= 4 && charNum <= 4 && charNum >= 3)) {
                        ...
                        zjpeople++;
                        getChannelServer().broadcastPacket(MaplePacketCreator.serverNotice(6,
                                "[赌博系统]" + Integer.parseInt(ServerProperties.getProperty("RoyMS.赌博B赔率", "3")) + "倍中奖名单:"
                                        + zjNames6 + "！恭喜他(她)吧"));
                    } else {
                        chr.dropMessage(1, "本期号码：【" + this.zjNum + "】\r\n对不起您没有中奖，请继续努力");
                    }
                ...
                if (charType == 6) {
                    ...
                    if (this.zjNum == charNum) {
                        ...
                        zjpeople++;
                        getChannelServer().broadcastPacket(MaplePacketCreator.serverNotice(6,
                                "[赌博系统]" + Integer.parseInt(ServerProperties.getProperty("RoyMS.赌博C赔率", "6")) + "倍中奖名单:"
                                        + zjNames6 + "！恭喜他(她)吧"));
                        continue;
                    }
                    chr.dropMessage(1, "本期号码：【" + this.zjNum + "】\r\n对不起您没有中奖，请继续努力");
        ...
        getChannelServer()
                .broadcastPacket(MaplePacketCreator.serverNotice(6, ServerProperties.getProperty("RoyMS.赌博公告")));
        if ("".equals(zjNames6))
            ;
    }
