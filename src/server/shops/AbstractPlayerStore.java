...
public abstract class AbstractPlayerStore extends AbstractMapleMapObject implements IMaplePlayerShop {
    ...
    public AbstractPlayerStore(final MapleCharacter owner, final int itemId, final String desc, final String pass,
            final int slots) {
        this.open = false;
        ...
        try {
            ...
            if (rs.next()) {
                ...
                ps2 = con.prepareStatement(
                        "INSERT INTO hiredmerch (characterid, accountid, Mesos, map, channel, time) VALUES (?, ?, ?, ?, ?, ?)",
                        1);
            ...
            for (final MaplePlayerShopItem pItems : this.items) {
                if (pItems.item != null && pItems.bundles > 0) {
                    ...
                    iters.add(new Pair<IItem, MapleInventoryType>(item,
    ...
    public void removeAllVisitors(final int error, final int type) {
        for (int i = 0; i < this.chrs.length; ++i) {
            ...
            if (visitor != null) {
                ...
                this.broadcastToVisitors(PlayerShopPacket.shopVisitorLeave(this.getVisitorSlot(visitor)),
                        this.getVisitorSlot(visitor));
