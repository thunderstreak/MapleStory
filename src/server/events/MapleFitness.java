        ...
        this.msgSchedule = Timer.EventTimer.getInstance().register(new Runnable() {
            ...
            public void run() {
                ...
                if (timeLeft > 9000L && timeLeft < 11000L) {
                    ...
                    MapleFitness.this.broadcast(MaplePacketCreator.serverNotice(0,
                            "第4阶段，是最后一项 [冒险岛体能测试]. 请不要在最后一刻放弃，拼尽全力. 丰厚的奖励在最高层等着你哦!"));
                    ...
                    MapleFitness.this.broadcast(
                            MaplePacketCreator.serverNotice(0, "请记住，如果你在活动期间死掉了，你会从游戏中淘汰. 如果你想补充HP，使用药水或移动之前先恢复HP。"));
                    ...
                    MapleFitness.this.broadcast(MaplePacketCreator.serverNotice(0,
                            "每个人都该参加 [冒险岛体能测试] 参加这个项目，无论完成的顺序，都会获得奖励，所以只是娱乐，慢慢来，清除了4个阶段。"));
                    ...
                    MapleFitness.this.broadcast(
                            MaplePacketCreator.serverNotice(0, "[冒险岛体能测试]有4个阶段，如果你碰巧在游戏过程中死亡，你会从比赛被淘汰，所以请注意这一点。"));
                }
            }
        }, 90000L);
    }
