    ...
    public static class MapleSnowballs {
        ...
        public static void hitSnowball(final MapleCharacter chr) {
            ...
            if (ball != null && !ball.isInvis()) {
                ...
                if (!snowman) {
                    final int damage = ((Math.random() < 0.01
                            || (chr.getPosition().x > ball.getLeftX() && chr.getPosition().x < ball.getRightX()))
                            && ball.isHittable()) ? 10 : 0;
                        ...
                        if (ball.getPosition() == 255 || ball.getPosition() == 511 || ball.getPosition() == 767) {
                            ...
                            for (int i = 0; i < 2; ++i) {
                                ...
                                map.broadcastMessage(
                                        MaplePacketCreator.rollSnowball(i + 2, sb.getSnowBall(0), sb.getSnowBall(1)));
                            }
                            chr.getMap().broadcastMessage(MaplePacketCreator.serverNotice(6,
                                    "[恭喜] " + ((team == 0) ? "蓝队" : "红队") + " 赢得胜利!"));
                            for (final MapleCharacter chrz : chr.getMap().getCharactersThreadsafe()) {
                                if ((team == 0 && chrz.getPosition().y > -80)
                                        || (team == 1 && chrz.getPosition().y <= -80)) {
                                    sb.givePrize(chrz);
                                }
                                sb.warpBack(chrz);
                            }
                    ...
                    if (damage > 0) {
                        chr.getMap().broadcastMessage(
                                MaplePacketCreator.rollSnowball(0, sb.getSnowBall(0), sb.getSnowBall(1)));
