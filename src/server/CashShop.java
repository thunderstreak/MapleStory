...
public class CashShop implements Serializable {
    ...
    public IItem toItem(final CashItemInfo cItem, int uniqueid, final String gift) {
        ...
        if (GameConstants.isPet(cItem.getId())) {
            period = 90L;
        } else if (cItem.getId() < 5210000 || cItem.getId() > 5360099 || cItem.getId() == 5220007
                || cItem.getId() == 5220008) {
            period = 0L;
        }
        IItem ret = null;
    ...
    public void gift(final int recipient, final String from, final String message, final int sn, final int uniqueid) {
        try {
            final PreparedStatement ps = DatabaseConnection.getConnection()
                    .prepareStatement("INSERT INTO `gifts` VALUES (DEFAULT, ?, ?, ?, ?, ?)");
    ...
    public void save() throws SQLException {
        ...
        for (final IItem item : this.inventory) {
            itemsWithType
                    .add(new Pair<IItem, MapleInventoryType>(item, GameConstants.getInventoryType(item.getItemId())));
        }
