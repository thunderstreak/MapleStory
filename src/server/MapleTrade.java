...
public class MapleTrade {
    ...
    public static void startTrade(final MapleCharacter c) {
        if (c.getTrade() == null) {
            ...
            c.getClient().getSession()
                    .write(MaplePacketCreator.getTradeStart(c.getClient(), c.getTrade(), (byte) 0, false));
        } else {
    ...
    public static void start现金交易(final MapleCharacter c) {
        if (c.getTrade() == null) {
            c.setTrade(new MapleTrade((byte) 0, c));
            c.getClient().getSession()
                    .write(MaplePacketCreator.getTradeStart(c.getClient(), c.getTrade(), (byte) 0, true));
        } else {
            c.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "不能同时做多件事情。"));
        }
    }
        ...
        if (c1.getMap().getId() == c2.getMap().getId()) {
            if (c1.getTrade() != null && c1.getTrade().getPartner() == c2.getTrade() && c2.getTrade() != null
                ...
                c1.getClient().getSession()
                        .write(MaplePacketCreator.getTradeStart(c1.getClient(), c1.getTrade(), (byte) 1, true));
            } else {
    ...
    public static void visitTrade(final MapleCharacter c1, final MapleCharacter c2) {
        if (c1.getMap().getId() == c2.getMap().getId()) {
            if (c1.getTrade() != null && c1.getTrade().getPartner() == c2.getTrade() && c2.getTrade() != null
                ...
                c2.getClient().getSession().write(MaplePacketCreator.getTradePartnerAdd(c1));
                c1.getClient().getSession()
                        .write(MaplePacketCreator.getTradeStart(c1.getClient(), c1.getTrade(), (byte) 1, false));
            } else {
                c1.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "对方已经取消了交易。"));
            }
        } else {
    ...
    public void CompleteTrade() {
        ...
        if (this.exchangeMeso > 0) {
            this.chr.get().gainMeso(this.exchangeMeso - GameConstants.getTaxAmount(this.exchangeMeso), false, true,
                    false);
        }
        this.exchangeMeso = 0;
    ...
    public void chat(final String message) {
        ...
        if (this.partner != null) {
            this.partner.getChr().getClient().getSession()
                    .write(PlayerShopPacket.shopChat(this.chr.get().getName() + " : " + message, 1));
        }
    }
    ...
    public boolean setItems(final MapleClient c, final IItem item, byte targetSlot, final int quantity) {
        ...
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        if (target == -1 || GameConstants.isPet(item.getItemId()) || this.isLocked()
    ...
    private int check() {
        ...
        for (final IItem item : this.exchangeItems) {
            ...
            if (ii.isPickupRestricted(item.getItemId())
                    && this.chr.get().getInventory(GameConstants.getInventoryType(item.getItemId()))
