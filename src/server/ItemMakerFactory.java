...
public class ItemMakerFactory {
    ...
    protected ItemMakerFactory() {
        ...
        final MapleData info = MapleDataProviderFactory
                .getDataProvider(new File(System.getProperty("wzPath") + "/Etc.wz")).getData("ItemMake.img");
            ...
            switch (type) {
                        ...
                        for (final MapleData rewardNRecipe : itemFolder.getChildren()) {
                            for (final MapleData ind : rewardNRecipe.getChildren()) {
                                if (rewardNRecipe.getName().equals("randomReward")) {
                                    ret.addRandomReward(MapleDataTool.getInt("item", ind, 0),
                                            MapleDataTool.getInt("prob", ind, 0));
                                } else {
                                    if (!rewardNRecipe.getName().equals("recipe")) {
                                        continue;
                                    }
                                    ret.addReqRecipe(MapleDataTool.getInt("item", ind, 0),
                ...
                case 16: {
                    for (final MapleData itemFolder : dataType.getChildren()) {
                        ...
                        final ItemMakerCreateEntry imt = new ItemMakerCreateEntry(cost, reqLevel, reqMakerLevel,
                                quantity, totalupgrades, stimulator);
                        for (final MapleData Recipe : itemFolder.getChildren()) {
                            for (final MapleData ind : Recipe.getChildren()) {
                                if (Recipe.getName().equals("recipe")) {
                                    imt.addReqItem(MapleDataTool.getInt("item", ind, 0),
                                            MapleDataTool.getInt("count", ind, 0));
                                }
                            }
                        }
    ...
    public static class ItemMakerCreateEntry {
        ...
        public ItemMakerCreateEntry(final int cost, final int reqLevel, final byte reqMakerLevel, final int quantity,
                final byte tuc, final int stimulator) {
