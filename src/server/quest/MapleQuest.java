...
public class MapleQuest implements Serializable {
    ...
    private static boolean loadQuest(final MapleQuest ret, final int id) throws NullPointerException {
                ...
                for (final MapleData startReq : startC) {
                    ...
                    if (req.getType().equals(MapleQuestRequirementType.mob)) {
                        for (final MapleData mob : startReq.getChildren()) {
                            ret.relevantMobs.put(MapleDataTool.getInt(mob.getChildByPath("id")),
        ...
        if (completeReqData != null) {
            ...
            if (completeC != null && completeC.size() > 0) {
                for (final MapleData completeReq : completeC) {
                    final MapleQuestRequirement req = new MapleQuestRequirement(ret,
                            MapleQuestRequirementType.getByWZName(completeReq.getName()), completeReq);
                    if (req.getType().equals(MapleQuestRequirementType.mob)) {
        ...
        if (startActData != null) {
            ...
            for (final MapleData startAct : startC2) {
                ret.startActs
                        .add(new MapleQuestAction(MapleQuestActionType.getByWZName(startAct.getName()), startAct, ret));
            }
        }
        ...
        if (completeActData != null) {
            ...
            for (final MapleData completeAct : completeC2) {
                ret.completeActs.add(new MapleQuestAction(MapleQuestActionType.getByWZName(completeAct.getName()),
                        completeAct, ret));
            }
        }
            ...
            for (final MapleData d : pquestInfo.getChildByPath("rank")) {
                ...
                for (final MapleData c : d) {
                    for (final MapleData b : c) {
                        pInfo.add(new Pair<String, Pair<String, Integer>>(c.getName(),
                                new Pair<String, Integer>(b.getName(), MapleDataTool.getInt(b, 0))));
                    }
                }
    ...
    public static void initQuests() {
        MapleQuest.questData = MapleDataProviderFactory
                .getDataProvider(new File(System.getProperty("wzPath") + "/Quest.wz"));
    ...
    public enum MedalQuest {
        ...
        MinarForest(29011, 29012, 70,
                new int[] { 240000000, 240010200, 240010800, 240020401, 240020101, 240030000, 240040400, 240040511,
                        240040521, 240050000 }),
        Sleepywood(29014, 29015, 50, new int[] { 105040300, 105070001, 105040305, 105090200, 105090300, 105090301,
