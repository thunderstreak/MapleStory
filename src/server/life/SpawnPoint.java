...
public class SpawnPoint extends Spawns {
    ...
    public boolean shouldSpawn() {
        return this.mobTime >= 0 && ((this.mobTime == 0 && !this.immobile) || this.spawnedMonsters.get() <= 0)
                && this.spawnedMonsters.get() <= 1 && this.nextPossibleSpawn <= System.currentTimeMillis();
    }
    ...
    public MapleMonster spawnMonster(final MapleMap map) {
        ...
        if (this.carnivalTeam > -1) {
            for (final MapleReactor r : map.getAllReactorsThreadsafe()) {
                if (r.getName().startsWith(String.valueOf(this.carnivalTeam))
                        && r.getReactorId() == 9980000 + this.carnivalTeam && r.getState() < 5) {
