...
public class MapleMonster extends AbstractLoadedMapleLife {
                        ...
                        for (final AttackingMapleCharacter cattacker : mattacker.getAttackers()) {
                            if (cattacker.getAttacker().getMap() == from.getMap()
                                ...
                                cattacker.getAttacker().getClient().getSession().write(MobPacket.showMonsterHP(
                                        this.getObjectId(), (int) Math.ceil(this.hp * 100.0 / this.getMobMaxHp())));
                            }
                        }
                    }
                }
                ...
                if (this.sponge.get() == null && this.hp > 0L) {
                    switch (this.stats.getHPDisplayType()) {
                        ...
                        case 2: {
                            ...
                            this.map.broadcastMessage(MobPacket.showMonsterHP(this.getObjectId(),
                                    (int) Math.ceil(this.hp * 100.0 / this.getMobMaxHp())));
                        ...
                        case 3: {
                            for (final AttackerEntry mattacker2 : this.attackers) {
                                for (final AttackingMapleCharacter cattacker2 : mattacker2.getAttackers()) {
                                    if (cattacker2.getAttacker().getMap() == from.getMap()
                                            && cattacker2.getLastAttackTime() >= System.currentTimeMillis() - 4000L) {
                                        cattacker2.getAttacker().getClient().getSession()
                                                .write(MobPacket.showMonsterHP(this.getObjectId(),
                                                        (int) Math.ceil(this.hp * 100.0 / this.getMobMaxHp())));
                                    }
                                }
                            }
    ...
    public void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage,
            final int numExpSharers, final byte pty, final byte Class_Bonus_EXP_PERCENT, final byte 网吧特别经验_百分比,
        ...
        if (exp > 0) {
            ...
            exp = Math.min(Integer.MAX_VALUE,
                    exp * ((attacker.getLevel() < 10) ? GameConstants.getExpRate_Below10(attacker.getJob())
                            : ChannelServer.getInstance(this.map.getChannel()).getExpRate()));
    ...
    public int killBy(final MapleCharacter killer, final int lastSkill) {
        ...
        for (final AttackerEntry attackEntry2 : this.attackers) {
            final int baseExp = (int) Math
                    .ceil(totalBaseExp * (attackEntry2.getDamage() / (double) this.getMobMaxHp()));
            ...
            default: {
                ...
                if (oldSponge != null && oldSponge.isAlive()) {
                    ...
                    for (final MapleMapObject mon : this.map.getAllMonstersThreadsafe()) {
                        ...
                        if (mons.getObjectId() != oldSponge.getObjectId() && mons.getObjectId() != this.getObjectId()
    ...
    public final void resetShammos(final MapleClient c) {
        ...
        this.map.broadcastMessage(MaplePacketCreator.serverNotice(5,
    ...
    public void sendSpawnData(final MapleClient client) {
        ...
        if (this.lastNode >= 0 && this.getId() == 9300275 && this.map.getId() >= 921120100
                && this.map.getId() < 921120500) {
    ...
    public void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison,
            final long duration, final boolean venom) {
        this.applyStatus(from, status, poison, duration, venom, true);
    }

    public void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison,
            long duration, final boolean venom, final boolean checkboss) {
        if (!this.isAlive()) {
            return;
        }
        ...
        if (this.getId() == 9600000
                && (stat == MonsterStatus.冻结 || stat == MonsterStatus.中毒 || stat == MonsterStatus.眩晕)) {
            return;
        }
        if (this.stats.isBoss()) {
            ...
            if (checkboss && stat != MonsterStatus.速度 && stat != MonsterStatus.忍者伏击 && stat != MonsterStatus.中毒
                    && stat != MonsterStatus.物攻) {
                return;
            }
        }
        if ((this.stats.isFriendly() || this.isFake())
                && (stat == MonsterStatus.眩晕 || stat == MonsterStatus.速度 || stat == MonsterStatus.中毒)) {
            return;
        }
        ...
        if (poison && this.getHp() > 1L) {
            final int poisonDamage = (int) Math.min(32767L,
                    (long) (this.getMobMaxHp() / (70.0 - from.getSkillLevel(status.getSkill())) + 0.999));
            status.setValue(MonsterStatus.中毒, poisonDamage);
            status.setPoisonSchedule(
                    timerManager.register(new PoisonTask(poisonDamage, from, status, cancelTask, false), 1000L, 1000L));
            if (from.isGM()) {
                from.dropMessage(6, "正在给予 怪物: " + this.getId() + " BUFF状态: " + status.getStati().name() + " 是否中毒: "
                        + poison + " BUFF持续时间: " + duration);
            }
            if (duration >= 60000L) {
                duration = 10000L;
            }
            ...
            status.setPoisonSchedule(timerManager
                    .register(new PoisonTask(poisonDamage2, from, status, cancelTask, false), 1000L, 1000L));
        } else if (statusSkill == 4111003 || statusSkill == 14111001) {
            status.setPoisonSchedule(timerManager.schedule(
                    new PoisonTask((int) (this.getMobMaxHp() / 50.0 + 0.999), from, status, cancelTask, true), 3500L));
        } else if (statusSkill == 4121004 || statusSkill == 4221004) {
            int damage = (from.getStat().getStr() + from.getStat().getLuk()) * 2 * 0 + 100;
            status.setPoisonSchedule(
                    timerManager.register(new PoisonTask(damage, from, status, cancelTask, false), 1000L, 1000L));
            if (damage > 0) {
                if (damage >= this.hp) {
        ...
        if (this.getController() != null && !this.getController().isMapObjectVisible(this)) {
            this.getController().getClient().getSession()
                    .write(MobPacket.applyMonsterStatus(this.getObjectId(), status));
        }
        int aniTime = 0;
        if (skilz != null) {
    ...
    public void dispelSkill(final MobSkill skillId) {
        ...
        for (final Map.Entry<MonsterStatus, MonsterStatusEffect> effects : this.stati.entrySet()) {
            if (effects.getValue().getMobSkill() != null
                    && effects.getValue().getMobSkill().getSkillId() == skillId.getSkillId()) {
    ...
    public void applyMonsterBuff(final Map<MonsterStatus, Integer> effect, final int skillId, final long duration,
            final MobSkill skill, final List<Integer> reflection) {
        final Timer.MobTimer timerManager = Timer.MobTimer.getInstance();
        final Runnable cancelTask = new Runnable() {
            @Override
            public void run() {
        ...
        if (reflection.size() > 0) {
            ...
            for (final Map.Entry<MonsterStatus, Integer> z : effect.entrySet()) {
                this.map.broadcastMessage(
                        MobPacket.applyMonsterStatus(this.getObjectId(), z.getKey(), z.getValue(), skill),
                        this.getPosition());
                if (this.getController() != null && !this.getController().isMapObjectVisible(this)) {
                    this.getController().getClient().getSession()
                            .write(MobPacket.applyMonsterStatus(this.getObjectId(), z.getKey(), z.getValue(), skill));
                }
            }
        }
            ...
            for (final MonsterDropEntry d : dropEntry) {
                if (d.itemId > 0 && d.questid == 0 && d.itemId / 10000 != 238
                    ...
                    if (GameConstants.getInventoryType(d.itemId) == MapleInventoryType.EQUIP) {
                        ...
                        idrop = new Item(d.itemId, (short) 0,
                                (short) ((d.Maximum != 1) ? (Randomizer.nextInt(d.Maximum - d.Minimum) + d.Minimum)
                                        : 1),
                                (byte) 0);
                    }
                    this.stolen = d.itemId;
                    this.map.spawnMobDrop(idrop, this.map.calcDropPos(this.getPosition(), this.getTruePosition()), this,
                            chr, (byte) 0, (short) 0);
                    break;
                }
            }
    ...
    public void cancelStatus(final MonsterStatus stat) {
        ...
        if (con != null) {
            this.map.broadcastMessage(con, MobPacket.cancelMonsterStatus(this.getObjectId(), stat),
    ...
    private class PoisonTask implements Runnable {
        ...
        public void run() {
            ...
            if (MapleMonster.this.hp > 1L && damage > 0L) {
                ...
                if (this.shadowWeb) {
                    this.map.broadcastMessage(MobPacket.damageMonster(MapleMonster.this.getObjectId(), damage),
                            MapleMonster.this.getPosition());
                }
            }
        }
    }
    ...
    private class SingleAttackerEntry implements AttackerEntry {
        ...
        public void killedMob(final MapleMap map, final int baseExp, final boolean mostDamage, final int lastSkill) {
            ...
            if (chr != null && chr.isAlive()) {
                MapleMonster.this.giveExpToCharacter(chr, baseExp, mostDamage, 1, (byte) 0, (byte) 0, (byte) 0,
                        lastSkill);
            }
        }
        ...
        public void killedMob(final MapleMap map, final int baseExp, final boolean mostDamage, final int lastSkill) {
            ...
            for (final Map.Entry<MapleCharacter, OnePartyAttacker> attacker : this.resolveAttackers().entrySet()) {
                ...
                for (final MapleCharacter expReceiver : expApplicable) {
                    ...
                    expMap.put(expReceiver,
                            new ExpMap(iexp, (byte) (expApplicable.size() + added_组队经验值), Class_Bonus_EXP, 网吧特别经验));
                }
            }
            for (final Map.Entry<MapleCharacter, ExpMap> expReceiver2 : expMap.entrySet()) {
                final ExpMap expmap = expReceiver2.getValue();
                MapleMonster.this.giveExpToCharacter(expReceiver2.getKey(), expmap.exp,
                        mostDamage && expReceiver2.getKey() == highest, expMap.size(), expmap.ptysize,
                        expmap.Class_Bonus_EXP, expmap.网吧特别经验, lastSkill);
            }
        }
