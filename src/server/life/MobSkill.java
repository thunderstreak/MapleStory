...
public class MobSkill {
    ...
    public boolean checkCurrentBuff(final MapleCharacter player, final MapleMonster monster) {
        ...
        switch (this.skillId) {
            ...
            case 144:
            case 145: {
                stop = (monster.isBuffed(MonsterStatus.免疫伤害) || monster.isBuffed(MonsterStatus.免疫魔法攻击)
                        || monster.isBuffed(MonsterStatus.免疫物理攻击));
                break;
            }
            case 200: {
        ...
        switch (this.skillId) {
            ...
            case 129: {
                ...
                if (monster.getEventInstance() != null
                        && monster.getEventInstance().getName().indexOf("BossQuest") != -1) {
                    break;
                }
            ...
            case 131: {
                if (monster != null) {
                    monster.getMap().spawnMist(
                            new MapleMist(this.calculateBoundingBox(monster.getPosition(), true), monster, this),
                            this.x * 10, false);
                    break;
                }
                break;
                    ...
                    switch (mobId) {
                        ...
                        case 8510100: {
                            if (Math.ceil(Math.random() * 5.0) == 1.0) {
                                ypos = 78;
                                xpos = (int) (0.0 + Math.ceil(Math.random() * 5.0))
                                        + ((Math.ceil(Math.random() * 2.0) == 1.0) ? 180 : 0);
                                break;
                            }
                            xpos = (int) (monster.getPosition().getX() + Math.ceil(Math.random() * 1000.0) - 500.0);
                            break;
        ...
        if (stats.size() > 0 && monster != null) {
            if (this.lt != null && this.rb != null && skill) {
                for (final MapleMapObject mons2 : this.getObjectsInRange(monster, MapleMapObjectType.MONSTER)) {
                    ((MapleMonster) mons2).applyMonsterBuff(stats, this.getSkillId(), this.getDuration(), this,
                            reflection);
                }
            } else {
