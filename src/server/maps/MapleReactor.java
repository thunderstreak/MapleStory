...
public class MapleReactor extends AbstractMapleMapObject {
        ...
        if (this.stats.getType(this.state) < 999 && this.stats.getType(this.state) != -1) {
            ...
            if (pass || (charPos != 0 && charPos != 2)) {
                ...
                if (this.stats.getNextState(this.state) == -1 || this.stats.getType(this.state) == 999) {
                    if ((this.stats.getType(this.state) < 100 || this.stats.getType(this.state) == 999)
                            && this.delay > 0) {
                    ...
                    boolean done = false;
                    this.map.broadcastMessage(MaplePacketCreator.triggerReactor(this, stance));
                    if (this.state == this.stats.getNextState(this.state) || this.rid == 2618000
                            || this.rid == 2309000) {
                        if (this.rid > 200011) {
                            ReactorScriptManager.getInstance().act(c, this);
                        }
    ...
    public String toString() {
        return "Reactor " + this.getObjectId() + " of id " + this.rid + " at position " + this.getPosition().toString()
                + " state" + this.state + " type " + this.stats.getType(this.state);
    }
