...
public class MapleMap {
    ...
    public MapleMap(final int mapid, final int channel, final int returnMapId, final float monsterRate) {
        ...
        final EnumMap<MapleMapObjectType, LinkedHashMap<Integer, MapleMapObject>> objsMap = new EnumMap<MapleMapObjectType, LinkedHashMap<Integer, MapleMapObject>>(
                MapleMapObjectType.class);
        final EnumMap<MapleMapObjectType, ReentrantReadWriteLock> objlockmap = new EnumMap<MapleMapObjectType, ReentrantReadWriteLock>(
                MapleMapObjectType.class);
        ...
        this.mapobjects = Collections.unmodifiableMap(
                (Map<? extends MapleMapObjectType, ? extends LinkedHashMap<Integer, MapleMapObject>>) objsMap);
        this.mapobjectlocks = Collections
                .unmodifiableMap((Map<? extends MapleMapObjectType, ? extends ReentrantReadWriteLock>) objlockmap);
    }
    ...
    private void spawnAndAddRangedMapObject(final MapleMapObject mapobject, final DelayedPacketCreation packetbakery,
        ...
        try {
            for (final MapleCharacter chr : this.characters) {
                if ((condition == null || condition.canSpawn(chr)) && !chr.isClone()
                        && chr.getPosition().distanceSq(mapobject.getPosition()) <= GameConstants.maxViewRangeSq()) {
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean instanced) {
        if (mob == null || chr == null || ChannelServer.getInstance(this.channel) == null || this.dropsDisabled
                || mob.dropsDisabled() || chr.getPyramidSubway() != null) {
            return;
        }
        ...
        final int tempmeso = Math.min(30000,
                (int) (mesoDecrease * mob.getStats().getExp() * (1.0 + Math.random() * 5.0) / 30.0));
        if (tempmeso > 0) {
            pos.x = Math.min(Math.max(mobpos - 25 * (d / 2), this.footholds.getMinDropX() + 25),
                    this.footholds.getMaxDropX() - d * 25);
            this.spawnMobMesoDrop((int) (tempmeso * (chr.getStat().mesoBuff / 100.0) * chr.getDropMod() * cmServerrate),
                    this.calcDropPos(pos, mob.getPosition()), mob, chr, false, droptype);
        }
        ...
        final int cashModifier = (int) (mob.getStats().isBoss() ? 0L
                : (mob.getMobExp() / 1000 + mob.getMobMaxHp() / 10000L));
        for (final MonsterGlobalDropEntry de2 : globalEntry) {
            if (Randomizer.nextInt(999999) < de2.chance
                    && (de2.continent < 0 || (de2.continent < 10 && this.mapid / 100000000 == de2.continent)
                            || (de2.continent < 100 && this.mapid / 10000000 == de2.continent)
                            || (de2.continent < 1000 && this.mapid / 1000000 == de2.continent))) {
                if (droptype == 3) {
                    pos.x = mobpos + ((d % 2 == 0) ? (40 * (d + 1) / 2) : (-(40 * (d / 2))));
                } else {
                    pos.x = mobpos + ((d % 2 == 0) ? (25 * (d + 1) / 2) : (-(25 * (d / 2))));
                ...
                if (GameConstants.getInventoryType(de2.itemId) == MapleInventoryType.EQUIP) {
                    ...
                    idrop = new Item(de2.itemId, (short) 0,
                            (short) ((de2.Maximum != 1) ? (Randomizer.nextInt(de2.Maximum - de2.Minimum) + de2.Minimum)
                                    : 1),
                            (byte) 0);
                }
                this.spawnMobDrop(idrop, this.calcDropPos(pos, mob.getPosition()), mob, chr,
                        (byte) (de2.onlySelf ? 0 : droptype), de2.questid);
                ++d;
            }
        }
    }
    ...
    public void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops,
            final boolean second, final byte animation, final int lastSkill) {
        this.spawnedMonstersOnMap.decrementAndGet();
        this.removeMapObject(monster);
        monster.killed();
                    ...
                    if (mc.isAlive()) {
                        ...
                        switch (monster.getId()) {
                            ...
                            case 8820001: {
                                ...
                                this.broadcastMessage(mc, MaplePacketCreator.showBuffeffect(mc.getId(), buffid, 11),
                                        false);
                                continue;
                            }
                        }
        ...
        if (mobid == 8810018 && this.mapid == 240060200) {
            World.Broadcast.broadcastMessage(MaplePacketCreator
                    .serverNotice(6, "经过无数次的挑战，" + chr.getName() + "所带领的队伍终于击败了暗黑龙王的远征队！你们才是龙之林的真正英雄~").getBytes());
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator
                    .serverNotice(6, "经过无数次的挑战，" + chr.getName() + "所带领的队伍终于击败了混沌暗黑龙王的远征队！你们才是龙之林的真正英雄~").getBytes());
            if (this.speedRunStart > 0L) {
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator
                    .serverNotice(6, chr.getName() + "经过带领的队伍经过无数次的挑战，终于击败了时间的宠儿－品克缤的远征队！你们才是时间神殿的真正英雄~").getBytes());
            if (this.speedRunStart > 0L) {
        ...
        if (type != SpeedRunType.NULL && this.speedRunStart > 0L && this.speedRunLeader.length() > 0) {
            ...
            this.broadcastMessage(MaplePacketCreator.serverNotice(5,
                    this.speedRunLeader + "'远征队花了 " + time + " 时间打败了 " + type + "!"));
        ...
        if (mobid == 8820008) {
            ...
            for (final MapleMapObject mmo : this.getAllMonstersThreadsafe()) {
                final MapleMonster mons3 = (MapleMonster) mmo;
                if (mons3.getId() != 8820000 && mons3.getObjectId() != monster.getObjectId()
                        && mons3.getLinkOid() != monster.getObjectId()) {
                    this.killMonster(mons3, chr, false, false, animation);
                }
            }
        }
    ...
    public boolean containsNPC(int npcid) {
        ...
        try {
            Iterator<MapleMapObject> itr = ((LinkedHashMap) this.mapobjects.get(MapleMapObjectType.NPC)).values()
                    .iterator();
            while (itr.hasNext()) {
    ...
    public MapleMonster getMonsterById(int id) {
        ...
        try {
            ...
            Iterator<MapleMapObject> itr = ((LinkedHashMap) this.mapobjects.get(MapleMapObjectType.MONSTER)).values()
                    .iterator();
            while (itr.hasNext()) {
    ...
    public MapleReactor getReactorById(int id) {
        ...
        try {
            ...
            Iterator<MapleMapObject> itr = ((LinkedHashMap) this.mapobjects.get(MapleMapObjectType.REACTOR)).values()
                    .iterator();
            while (itr.hasNext()) {
    ...
    public final void spawnZakum(final int x, final int y) {
        ...
        final int[] array;
        final int[] zakpart = array = new int[] { 8800003, 8800004, 8800005, 8800006, 8800007, 8800008, 8800009,
                8800010 };
    ...
    private void checkRemoveAfter(final MapleMonster monster) {
        ...
        if (ra > 0) {
            Timer.MapTimer.getInstance().schedule(new Runnable() {
                ...
                public void run() {
                    if (monster != null
                            && monster == MapleMap.this.getMapObject(monster.getObjectId(), monster.getType())) {
    ...
    public void spawnDoor(final MapleDoor door) {
        this.spawnAndAddRangedMapObject(door, new DelayedPacketCreation() {
            ...
            public void sendPackets(final MapleClient c) {
                c.getSession()
                        .write(MaplePacketCreator.spawnDoor(door.getOwner().getId(), door.getTargetPosition(), false));
                if (door.getOwner().getParty() != null && (door.getOwner() == c.getPlayer()
                        || door.getOwner().getParty().containsMembers(new MaplePartyCharacter(c.getPlayer())))) {
                    c.getSession().write(MaplePacketCreator.partyPortal(door.getTown().getId(),
                            door.getTarget().getId(), door.getSkill(), door.getTargetPosition()));
                }
                c.getSession().write(MaplePacketCreator.spawnPortal(door.getTown().getId(), door.getTarget().getId(),
                        door.getSkill(), door.getTargetPosition()));
                    ...
                    public void run() {
                        for (MapleMapObject mo : MapleMap.this.getMapObjectsInRect(mist.getBox(),
                            ...
                            if (mist.makeChanceResult() && !((MapleMonster) mo).isBuffed(MonsterStatus.中毒))
                                ((MapleMonster) mo)
                                        .applyStatus(owner,
                                                new MonsterStatusEffect(MonsterStatus.中毒, Integer.valueOf(1),
    ...
    public void disappearingItemDrop(final MapleMapObject dropper, final MapleCharacter owner, final IItem item,
        ...
        this.broadcastMessage(MaplePacketCreator.dropItemFromMapObject(drop, dropper.getPosition(), droppos, (byte) 3),
                drop.getPosition());
    }
    ...
    public void spawnMesoDrop(final int meso, final Point position, final MapleMapObject dropper,
        ...
        this.spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            ...
            public void sendPackets(final MapleClient c) {
                c.getSession().write(
                        MaplePacketCreator.dropItemFromMapObject(mdrop, dropper.getPosition(), droppos, (byte) 1));
            }
        }, null);
        if (!this.everlast) {
    ...
    public void spawnMobDrop(final IItem idrop, final Point dropPos, final MapleMonster mob, final MapleCharacter chr,
        ...
        this.spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            ...
            public void sendPackets(final MapleClient c) {
                if (questid <= 0 || c.getPlayer().getQuestStatus(questid) == 1) {
                    c.getSession().write(
                            MaplePacketCreator.dropItemFromMapObject(mdrop, mob.getPosition(), dropPos, (byte) 1));
                }
            }
        }, null);
        mdrop.registerExpire(120000L);
    ...
    public void spawnItemDrop(final MapleMapObject dropper, final MapleCharacter owner, final IItem item,
            final Point pos, final boolean ffaDrop, final boolean playerDrop) {
        final Point droppos = this.calcDropPos(pos, pos);
        ...
        this.spawnAndAddRangedMapObject(drop, new DelayedPacketCreation() {
            ...
            public void sendPackets(final MapleClient c) {
                c.getSession().write(
                        MaplePacketCreator.dropItemFromMapObject(drop, dropper.getPosition(), droppos, (byte) 1));
            }
        }, null);
        ...
        try {
            for (final MapleMapObject o : this.mapobjects.get(MapleMapObjectType.REACTOR).values()) {
                ...
                if (react.getReactorType() == 100
                        && GameConstants.isCustomReactItem(react.getReactorId(), item.getItemId(),
                                react.getReactItem().getLeft())
                        && react.getReactItem().getRight() == item.getQuantity()
                        && react.getArea().contains(drop.getPosition()) && !react.isTimerActive()) {
    ...
    public void returnEverLastItem(final MapleCharacter chr) {
        for (final MapleMapObject o : this.getAllItemsThreadsafe()) {
            ...
            if (item.getOwner() == chr.getId()) {
                ...
                this.broadcastMessage(MaplePacketCreator.removeItemFromMap(item.getObjectId(), 2, chr.getId()),
                        item.getPosition());
                if (item.getMeso() > 0) {
    ...
    public void addPlayer(final MapleCharacter chr) {
        ...
        if (this.mapid == 109080000 || this.mapid == 109080001 || this.mapid == 109080002 || this.mapid == 109080003
                || this.mapid == 109080010 || this.mapid == 109080011 || this.mapid == 109080012) {
        ...
        for (final MaplePet pet : chr.getPets()) {
            if (pet.getSummoned()) {
                ...
                chr.getClient().getSession().write(PetPacket.updatePet(pet,
        ...
        if ((chr.getMapId() >= 140090100 && chr.getMapId() <= 140090500)
                || (chr.getJob() == 1000 && chr.getMapId() != 130030000)) {
        ...
        if (this.getSquadBegin() != null && this.getSquadBegin().getTimeLeft() > 0L
            ...
            chr.getClient().getSession()
                    .write(MaplePacketCreator.getClock((int) (this.getSquadBegin().getTimeLeft() / 1000L)));
            if (ServerConstants.封包显示 || 进入地图开启显示数据) {
        ...
        if (chr.getEventInstance() != null && chr.getEventInstance().isTimerStarted() && !chr.isClone()) {
            chr.getClient().getSession()
                    .write(MaplePacketCreator.getClock((int) (chr.getEventInstance().getTimeLeft() / 1000L)));
            if (ServerConstants.封包显示 || 进入地图开启显示数据) {
        ...
        if (this.getNumMonsters() > 0 && (this.mapid == 280030001 || this.mapid == 240060201 || this.mapid == 280030000
                || this.mapid == 280030100 || this.mapid == 240060200 || this.mapid == 220080001
                || this.mapid == 541020800 || this.mapid == 541010100)) {
    ...
    public void doShrine(final boolean spawned) {
        ...
        final MapleSquad sqd = this.getSquadByMap();
        final int mode = (this.mapid == 280030000 || this.mapid == 280030100) ? 1
                : ((this.mapid == 280030001) ? 2 : ((this.mapid == 240060200 || this.mapid == 240060201) ? 3 : 0));
    ...
    public void removePlayer(final MapleCharacter chr) {
        ...
        try {
            for (final MapleSummon summon : ss) {
                ...
                if (summon.getMovementType() == SummonMovementType.不会移动
    ...
    public List<MapleMapObject> getAllPlayers() {
        return this.getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY,
    ...
    private void broadcastMessage(final MapleCharacter source, final MaplePacket packet, final double rangeSq,
    ...
    private void sendObjectPlacement(final MapleCharacter chr) {
        ...
        for (final MapleMapObject o : this.getMapObjectsInRange(chr.getTruePosition(), GameConstants.maxViewRangeSq(),
    ...
    public void addAreaMonsterSpawn(final MapleMonster monster, Point pos1, Point pos2, Point pos3, final int mobTime,
            final String msg) {
        ...
        if (pos1 == null && pos2 == null && pos3 == null) {
            System.out.println(
                    "WARNING: mapid " + this.mapid + ", monster " + monster.getId() + " could not be spawned.");
            return;
        }
        if (pos1 != null) {
            if (pos2 == null) {
    ...
    public void updateMapObjectVisibility(final MapleCharacter chr, final MapleMapObject mo) {
        ...
        if (!chr.isMapObjectVisible(mo)) {
            if (mo.getType() == MapleMapObjectType.MIST || mo.getType() == MapleMapObjectType.SUMMON
                    || mo.getPosition().distanceSq(chr.getPosition()) <= GameConstants.maxViewRangeSq()) {
                chr.addVisibleMapObject(mo);
                mo.sendSpawnData(chr.getClient());
            }
        } else if (mo.getType() != MapleMapObjectType.MIST && mo.getType() != MapleMapObjectType.SUMMON
                && mo.getPosition().distanceSq(chr.getPosition()) > GameConstants.maxViewRangeSq()) {
            chr.removeVisibleMapObject(mo);
            mo.sendDestroyData(chr.getClient());
        } else if (mo.getType() == MapleMapObjectType.MONSTER
                && chr.getTruePosition().distanceSq(mo.getPosition()) <= GameConstants.maxViewRangeSq()) {
    ...
    public void movePlayer(final MapleCharacter player, final Point newPosition) {
        ...
        if (!player.isClone()) {
            try {
                ...
                for (final MapleMapObject mo2 : this.getMapObjectsInRange(player.getPosition(),
    ...
    public void AutoNx(int dy) {
        ...
        for (final MapleCharacter chr : this.characters) {
            ...
            // chr.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "[系统奖励]
            // 随机获得[" + dy + "] 点券奖励!"));
            // chr.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "[系统奖励]
            // 随机获得[" + dy + "] 抵用奖励!"));
            chr.getClient().getSession()
                    .write(MaplePacketCreator.serverNotice(5, "[系统奖励] 随机获得[" + chr.getLevel() * dy + "] 经验!"));
            // chr.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "[系统奖励]
            // 随机获得[" + dy + "] 点券 和[" + dy + "] 抵用奖励! 挂机获得[" + chr.getLevel() * dy + "]
            // 经验!"));
        }
    }
    ...
    public void getRankAndAdd(final String leader, final String time, final SpeedRunType type, final long timz,
            final Collection<String> squad) {
        try {
    ...
    public void broadcastGMMessage(final MapleCharacter source, final MaplePacket packet,
            final boolean repeatToSource) {
        this.broadcastGMMessage(repeatToSource ? null : source, packet, Double.POSITIVE_INFINITY, source.getPosition());
    }

    private void broadcastGMMessage(final MapleCharacter source, final MaplePacket packet, final double rangeSq,
            final Point rangedFrom) {
        this.charactersLock.readLock().lock();
        try {
            if (source == null) {
    ...
    public boolean canSpawn() {
        return this.lastSpawnTime > 0L && this.isSpawns
                && this.lastSpawnTime + this.createMobInterval < System.currentTimeMillis();
    }
    ...
    public void KillFk(final boolean animate) {
        final List<MapleMapObject> monsters = this.getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY,
    ...
    public int mobCount() {
        final List<MapleMapObject> mobsCount = this.getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY,
                Arrays.asList(MapleMapObjectType.MONSTER));
        return mobsCount.size();
    }

    public int playerCount() {
        final List<MapleMapObject> players = this.getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY,
                Arrays.asList(MapleMapObjectType.PLAYER));
        return players.size();
    }
    ...
    public void killMonster_2(final MapleMonster monster) {
        ...
        this.spawnedMonstersOnMap.decrementAndGet();
        monster.setHp(0L);
        this.broadcastMessage(MobPacket.killMonster(monster.getObjectId(),
    ...
    private class ActivateItemReactor implements Runnable {
        ...
        public void run() {
            if (this.mapitem != null
                    && this.mapitem == MapleMap.this.getMapObject(this.mapitem.getObjectId(), this.mapitem.getType())) {
