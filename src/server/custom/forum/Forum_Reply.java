...
public class Forum_Reply {
    ...
    public Forum_Reply(final int replyId, final int threadId, final int characterId, final String characterName,
            final String releaseTime, final String news) {
    ...
    public static ArrayList<Forum_Reply> loadAllReply() {
        ...
        try {
            ...
            while (rs.next()) {
                Forum_Reply.allReply.add(new Forum_Reply(rs.getInt("rid"), rs.getInt("tid"), rs.getInt("cid"),
                        rs.getString("cname"), rs.getString("time"), rs.getString("news")));
            }
            rs.close();
            ps.close();
            return Forum_Reply.allReply;
    ...
    public static Forum_Reply getReplyByNameToSql(final int tid, final String news) {
        ...
        try {
            ...
            final ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                return new Forum_Reply(rs.getInt("rid"), rs.getInt("tid"), rs.getInt("cid"), rs.getString("cname"),
                        rs.getString("time"), rs.getString("news"));
            }
        } catch (SQLException ex) {
            FileoutputUtil.outputFileError("logs/数据库异常.txt", ex);
        }
