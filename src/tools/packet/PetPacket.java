...
public class PetPacket {
    ...
    public static void addPetInfo(final MaplePacketLittleEndianWriter mplew, final MapleCharacter chr,
            final MaplePet pet, final boolean showpet) {
        if (showpet) {
            mplew.write(1);
            mplew.write(chr.getPetIndex(pet));
        }
    ...
    public static MaplePacket movePet(final int cid, final int pid, final byte slot,
    ...
    public static MaplePacket commandResponse(final int cid, final byte command, final byte slot, final boolean success,
            final boolean food) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (ServerConstants.调试输出封包) {
