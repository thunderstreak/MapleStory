...
public class ChairDump {
    public static void main(final String[] args) throws FileNotFoundException, IOException {
        ...
        sb.append("INSERT INTO shops (`shopid`, `npcid`) VALUES(").append(shopId).append(", ").append(npcId)
                .append(");\r\n");
        final int price = 1;
        for (final Pair<Integer, String> item : MapleItemInformationProvider.getInstance().getAllItems2()) {
            if (item.getLeft() >= 3010000 && item.getLeft() < 3020000) {
                sb.append("INSERT INTO shopitems (`shopid`, `itemid`, `price`, `position`) VALUES(").append(shopId)
                        .append(", ").append(item.getLeft()).append(", ").append(price).append(", 0);\r\n");
            }
        }
